...
| 260724-dnm | Cross-phase блокер из v1.0-аудита: `upload_receipt_entry` («💳 Оплата» re-entry) звал `start_payment_step` без `participant_type` → дефолт `"full"`, party-делегат видел full-тарифы или тихо помечался done без оплаты. Фикс: resolve `participant_type` через `get_user()` (fail-soft `"full"`, паттерн как `approve_user`), проброс 3-м аргументом. +1 регресс-тест. 27/27 test_payment_phase5 pass | 2026-07-24 | 663d697 | [260724-dnm-fix-party-payment-reentry-track](./quick/260724-dnm-fix-party-payment-reentry-track/) |
| 260724-cfn | Per-track party admin UI (WR-02/WR-05 из 05-REVIEW): `approve_text__party` editor в группе Party; track-свитчер full⇄party на экране «Тексты вопросов» (party-кнопки пишут `reg_prompt_{step}__party` через `reg_prompt_edit:{step}:party`); help-текст `payment_options` обновлён под track-синтаксис `label\|price\|track1,track2` (сверено с `_parse_options` payment.py). Рантайм override-семантика (D-05/D-15) не тронута — только способ задать через бота. 345 тестов pass | 2026-07-24 | 2b29037 | [260724-cfn-wr-02-wr-05-admin-ui-per-track-party-app](./quick/260724-cfn-wr-02-wr-05-admin-ui-per-track-party-app/) |
| 260724-dw1 | P0-хардининг из v1.0-аудита (4 фикса): (1) `aiogram>=3.0.0,<4.0.0` пин в requirements.txt (защита от breaking 4.x при fresh install); (2) resume file-size guard в `process_resume` (`_RESUME_MAX_BYTES`/`_resume_too_large`, отклоняет oversized до download/Nextcloud, паттерн как у чеков); (3) admin-alert при исчерпании ретраев Sheets append (`set_alert_bot` инъекция в main.py, fail-soft `_alert_admins_sheet_failure` в обоих append-путях, logger.error сохранён); (4) убран невалидный `version: 'version'` из docker-compose.yml. Nextcloud TLS-дефолт НЕ тронут (задокументировано). +37 тестов (resume guard + sheets alert), полный набор 359/359 pass | 2026-07-24 | 67660c0 | [260724-dw1-p0-hardening-pack](./quick/260724-dw1-p0-hardening-pack/) |
