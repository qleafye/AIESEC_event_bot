...
**Рефакторинг god-файлов (DX — не влезают в контекст):**
- [ ] Разбить `handlers/admin.py` (~3120) и `registration.py` (~2446) на связные подмодули, поведение байт-в-байт, порядок роутеров сохранён, опора на 359 тестов

> **Режим v2:** зафиксировано как scope/бэклог идей. Детальные фазы не планируются, пока нет точного ТЗ (решение 2026-07-24). Самодостаточны без ТЗ: реестр (keystone, стартовать первым) и рефакторинг god-файлов (инженерный; лучше после/в связке с реестром — реестр сам ужимает admin.py).
