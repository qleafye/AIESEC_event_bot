...
SETTINGS_SCHEMA = {
    ...
    "proxy_switch_dwell_seconds": {
        ...
        "prompt": (
            "Сколько секунд бот держится на канале (основном или резервном), прежде чем "
            "переключиться на следующий — даже если сетевые ошибки продолжают сыпаться. "
            "Защита от «дёрганья» между каналами: пока не истекла пауза, запрос просто "
            "падает и штатно повторяется — бот не мечется между каналами почём зря.\n\n"
