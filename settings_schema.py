...
SETTINGS_SCHEMA = {
    ...
    # Приёмка 17.09: делегат тапнул «Хочу свою ссылку» и увидел голую ссылку — непонятно, что
    # с ней делать, если не скопировать прямо сейчас. Пояснение — ГДЕ ссылку реально можно
    # найти потом: кнопка меню «🔗 Моя реферальная ссылка» в чате (`handlers/user_actions.py::
    # my_referral_link`, работает всегда, без гейта по статусу заявки). Раздел приложения НЕ
    # упомянут — здесь же, на терминальном экране «Заявка принята», ссылка показывается ОДИН
    # раз и без гейта по одобрению (`form_gate`, не `delegate_gate`); отдельного постоянного
    # места для неё в приложении нет (профиль/хаб её не показывают, проверено по коду).
    "miniapp_form_ambassador_link_note_text": {
        "type": "text", "group": "miniapp", "label": "🎁 Пояснение под ссылкой",
        "prompt": "Короткая строка рядом со ссылкой — где её найти потом (бот и приложение).",
        "default": "Эта ссылка всегда под рукой — в чате с ботом кнопка «🔗 Моя реферальная ссылка».",
        "per_city": True,
    },
