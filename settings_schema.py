...
SETTINGS_SCHEMA = {
    ...
    # Квик 260914-rgr (RGR-01..07, D-3): главный тумблер учёта чата делегатов. Дефолт "off" —
    # пока менеджер не включил его и не привязал чат, поведение бота не меняется ничем.
    # Кнопка-переключатель — НЕ на общем лендинге тумблеров (`settings_toggle_rows`), а на
    # самом экране «💬 Чат» (`chat_chat_tracking_toggle`, `handlers/admin_chat.py`) — там его
    # реально найдёт менеджер, ищущий именно чат, а не общий список из полусотни строк.
