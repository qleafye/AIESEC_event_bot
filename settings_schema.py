...
SETTINGS_SCHEMA = {
    ...
    # Phase 31 (31-03, D-30): машинная дата начала форума — ОТДЕЛЬНО от `event_date` выше.
    # `event_date` — свободный текст для делегата («12–13 сентября 2026»), его не трогаем.
    # `forum_date` — тип "date_only" (единственный день, без времени), по нему бот считает
    # возраст делегата НА ФОРУМЕ (правила автоотказа, план 31-08+) и годится фазе 32.
    "forum_date": {
        "type": "date_only", "group": "event", "label": "🗓 Дата начала форума",
        "prompt": (
            "Дата начала форума в формате ДД.ММ.ГГГГ, без времени. Например: 15.10.2026.\n\n"
            "По этой дате бот считает, сколько делегату будет лет НА ФОРУМЕ — от неё зависят "
            "правила автоотказа по возрасту."
        ),
        "default": None,
        "per_city": True,
    },
    ...
    "event_type": {
        ...
        "prompt": (
            ...
            "пресетом «🎓 Форум СкиллАп»).\n\nДля forum и conference бот сам "
            "включит/выключит модули оплаты и согласий — потом можно поправить кнопками "
            "выше."
        ),
        "default": None,
    },
    ...
    # Phase 31 (31-03, D-18): текст «заявку пересмотрят» — делегат видит его, когда менеджер
    # вернул заявку из журнала автоотказов на обычную модерацию (план 31-08+). Рядом с
    # reject_text — обе ветки одного смыслового блока «делегат узнаёт, что решил менеджер по
    # заявке». Реестр НЕ заводит второго ключа с причиной отказа (D-21) — тексты сработавших
    # правил просто добавляются к существующему reject_text, отдельного места под них нет и
    # не будет.
    "reject_rules_return_text": {
        "type": "text", "group": "reg", "label": "🔄 При возврате заявки на пересмотр",
        "prompt": (
            "Текст, который делегат увидит, когда менеджер вернул его заявку из журнала "
            "автоотказов на обычную модерацию (правила автоотказа больше не мешают — решение "
            "принимает человек). Оставьте пустым — стандартный текст."
        ),
        "default": None,
    },
    ...
    "nudge_text": {
        ...
        "prompt": (
            ...
            "Можно вставить {remaining} — бот подставит, сколько вопросов осталось этому "
            "делегату."
        ),
        "default": (
    ...
    # Phase 28 (28-09, SU-09, T-28-09-04): человеческое слово вместо числа в {remaining},
    # когда черновик не прочитался или остаток посчитать не удалось — делегат не должен
    # получить сообщение с фигурными скобками ни при каком сбое. Группа "apps" (не "reg") —
    # рядом с прочими текстами гейта «Заявки», физически тот же приём, что у nudge_text выше
    # (_APPS_FIELD_ORDER, не смена схемы-группы физического места).
    "nudge_remaining_fallback_text": {
        "type": "text", "group": "apps", "label": "⏰ Догонялка: слово вместо числа при сбое",
        "prompt": (
            "Когда бот не может посчитать, сколько вопросов осталось конкретному делегату "
            "(черновик анкеты не найден), в текст догонялки вместо числа подставится это "
            "слово.\n\nПример: несколько"
        ),
        "default": "несколько",
    },
    ...
    # Phase 28 (28-01, SU-03, R-A3 CONTEXT): множество статусов, при которых `course`/
    # `university`/`specialty`/`study_field` считаются условными по «учится» — пусто = прежнее
    # правило `startswith("Да")` байт-в-байт (D-06, reg_engine.is_studying).
    "edu_studying_statuses": {
        "type": "list", "group": "reg", "label": "🎓 Кто считается учащимся",
        "prompt": (
            "Статусы образования (из списка выше), при которых человек считается учащимся — "
            "каждый на отдельной строке. Пусто = как раньше (все статусы, начинающиеся с «Да»)."
        ),
        "default": None,
    },
    ...
    # Phase 28 (28-03, SU-02, A-06 CONTEXT): один ключ на мультивыбор с лимитом — int, пусто/0
    # = без лимита (D-06 «дефолт не меняем», пресет «СкиллАп» — план 28-11 — впишет числа
    # stack→5/goal→2, здесь остаётся прежнее поведение для YL/РилТолка). Не `per_city`
    # (28-UI-SPEC.md таблица новых ключей): менеджер задаёт число одно на все города.
    "reg_multi_max_goal": {
        "type": "int", "group": "reg", "label": "🎯 Максимум вариантов: Цель участия",
        "prompt": (
            "Сколько вариантов может отметить делегат. Например: 2. Оставьте пустым — "
            "без ограничения."
        ),
        "default": None,
    },
    "reg_multi_max_stack": {
        "type": "int", "group": "reg", "label": "🧰 Максимум вариантов: Стек",
        "prompt": (
            "Сколько вариантов может отметить делегат. Например: 5. Оставьте пустым — "
            "без ограничения."
        ),
        "default": None,
    },
    # Phase 30 (30-04, A2-05, T-30-10): максимум блоков `repeatable` — В ОТЛИЧИЕ от
    # `reg_multi_max_*` выше дефолт ОБЯЗАН быть конкретным числом, не `None` («без
    # ограничения» на JSON-список делегата — DoS, не просто визуал). Ключ назван по
    # `reg_repeatable_max_{step_key}` (`reg_engine.repeatable_max`) — второй repeatable-шаг
    # будущей фазы заведёт себе такой же ключ по этому же шаблону имени.
    "reg_repeatable_max_mini_portfolio": {
        "type": "int", "group": "reg", "label": "➕ Максимум блоков: Портфолио",
        "prompt": (
            "Сколько блоков «Название + описание» может добавить делегат. Например: 5."
        ),
        "default": 5,
    },
    # Phase 28 (28-03, SU-02): тексты лимита мультивыбора — подстановка ТОЛЬКО `.replace`
    # (`{max}`/`{selected}`), не `.format` (текст менеджера может содержать посторонние
    # фигурные скобки, T-073-03-05). `reg_multi_limit_hint_text` — единственный из трёх
    # обращённый к делегату per_city (подсказка формата под подписью поля); счётчик и ошибка —
    # нет (нейтральный технический текст, 28-UI-SPEC таблица новых ключей).
    "reg_multi_limit_hint_text": {
        "type": "text", "group": "reg", "label": "📝 Мультивыбор: подсказка лимита",
        "prompt": (
            "Подсказка под подписью поля с лимитом выбора. Подставляется {max} — например "
            "«Можно выбрать до {max}.»"
        ),
        "default": "Можно выбрать до {max}.",
        "per_city": True,
    },
    # Приёмка 17.09 (находка 3): подсказка «сколько нужно выбрать» ДО первого тапа —
    # `reg_engine.multi_requirement_hint`, одна и та же строка в приложении (дописана в
    # `spec.help`) и в чате (дописана к тексту вопроса). Два ключа, не один с условной
    # частью — менеджер редактирует каждую формулировку отдельно (за/без лимита звучат
    # по-разному по-русски, склеивать `.replace` было бы хрупко).
    "reg_multi_min_hint_text": {
        "type": "text", "group": "reg", "label": "📝 Мультивыбор: нужен минимум (без лимита)",
        "prompt": (
            "Подсказка до выбора на обязательном мультивыборе БЕЗ настроенного максимума. "
            "Подставляется {min} — например «Отметь вариантов: не меньше {min}.»"
        ),
        "default": "Отметь вариантов: не меньше {min}.",
        "per_city": True,
    },
    "reg_multi_min_max_hint_text": {
        "type": "text", "group": "reg", "label": "📝 Мультивыбор: нужно от — до (с лимитом)",
        "prompt": (
            "Подсказка до выбора на обязательном мультивыборе С настроенным максимумом — "
            "заменяет отдельную подсказку лимита. Подставляются {min} и {max} — например "
            "«Выбери от {min} до {max}.»"
        ),
        "default": "Выбери от {min} до {max}.",
        "per_city": True,
    },
    "reg_form_multi_pick_min_text": {
        "type": "text", "group": "reg_prompts", "label": "👉 «Выбери минимум» (multi, анкета 2.0)",
        "prompt": (
            "Текст неактивной кнопки «Дальше» в новой анкете, пока не набран минимум "
            "мультивыбора. Подставляется {min} — например «Выбери минимум {min}»."
        ),
        "default": "Выбери минимум {min}",
    },
    "reg_multi_limit_counter_text": {
        "type": "text", "group": "reg", "label": "📝 Мультивыбор: счётчик выбранного",
        "prompt": (
            "Текст счётчика над чекбоксами в приложении. Подставляются {selected} и {max} — "
            "например «Выбрано {selected} из {max}»"
        ),
        "default": "Выбрано {selected} из {max}",
    },
    "reg_multi_limit_error_text": {
        "type": "text", "group": "reg", "label": "📝 Мультивыбор: ошибка превышения лимита",
        "prompt": (
            "Текст при попытке выбрать больше, чем разрешено. Подставляется {max} — "
            "например «Можно выбрать не больше {max} вариантов.»"
        ),
        "default": "Можно выбрать не больше {max} вариантов.",
    },
    # Phase 28 (28-04, SU-04, СкиллАп 5, D-01/D-02/D-04): развилка резюме — вайтлист доменов
    # ссылки + подписи трёх кнопок R1 + маркеры/ошибка формата R2b + «Пропустить» второго
    # мини-подшага. Дефолты дословно из Copywriting Contract 28-UI-SPEC.md; ни один ключ не
    # per_city (таблица UI-SPEC явно помечает «нет» для всех восьми).
    "reg_resume_link_whitelist": {
        "type": "list", "group": "reg", "label": "🔗 Проверенные сайты для ссылки на резюме",
        "prompt": (
            "Домены, которые считаются проверенными (hh.ru, github.com и т.п.) — каждый на "
            "отдельной строке, без http://. Ссылки на другие сайты тоже принимаются, просто "
            "без пометки «проверено». Пусто = стандартный список."
        ),
        "default": None,
    },
    "reg_resume_fork_file_label": {
        "type": "text", "group": "reg", "label": "📎 Развилка резюме: кнопка «Файл»",
        "prompt": "Подпись первой кнопки на экране «Как пришлёте резюме?».",
        "default": "📎 Загрузить файл",
    },
    "reg_resume_fork_link_label": {
        "type": "text", "group": "reg", "label": "🔗 Развилка резюме: кнопка «Ссылка»",
        "prompt": "Подпись второй кнопки на экране «Как пришлёте резюме?».",
        "default": "🔗 Дать ссылку",
    },
    # Владелец 17.09: четвёртый вариант развилки — свободный текст об опыте вместо файла/ссылки.
    "reg_resume_fork_text_label": {
        "type": "text", "group": "reg", "label": "✍️ Развилка резюме: кнопка «Текстом»",
        "prompt": "Подпись третьей кнопки на экране «Как пришлёте резюме?».",
        "default": "✍️ Написать текстом",
    },
    "reg_resume_fork_text_prompt_text": {
        "type": "text", "group": "reg", "label": "✍️ Развилка резюме: вопрос «Опиши опыт текстом»",
        "prompt": "Текст вопроса, который делегат видит после тапа по кнопке «Написать текстом».",
        "default": (
            "Опиши свой опыт текстом: где работаешь или учишься, какие проекты, стажировки "
            "или волонтёрство есть, какие задачи решал(а) и каких результатов добился(ась) — "
            "коротко."
        ),
    },
    "reg_resume_fork_text_invalid_text": {
        "type": "text", "group": "reg", "label": "✍️ Развилка резюме: ошибка «пришли текстом»",
        "prompt": "Текст, если на ветке «Написать текстом» делегат прислал не текст (стикер/фото/файл).",
        "default": "Опиши опыт текстом ответом в чате.",
    },
    "reg_resume_fork_none_label": {
        "type": "text", "group": "reg", "label": "🙅 Развилка резюме: кнопка «Нет резюме»",
        "prompt": "Подпись четвёртой кнопки на экране «Как пришлёте резюме?».",
        "default": "🙅 У меня нет резюме",
    },
    "reg_resume_link_whitelist_hint_text": {
        "type": "text", "group": "reg", "label": "🔗 Ссылка на резюме: маркер «сайт проверенный»",
        "prompt": (
            "Подсказка под полем ссылки, когда домен есть в списке проверенных. "
            "Подставляется {domain}."
        ),
        "default": "{domain} — сайт из списка проверенных",
    },
    "reg_resume_link_other_hint_text": {
        "type": "text", "group": "reg", "label": "🔗 Ссылка на резюме: маркер «другой сайт»",
        "prompt": "Подсказка под полем ссылки, когда домен не из списка проверенных.",
        "default": "Личный сайт — тоже подойдёт",
    },
    ...
    "reg_mini_portfolio_skip_label": {
        ...
        "prompt": "Подпись кнопки-пропуска второго подшага мини-профиля (портфолио).",
        "default": "Пропустить",
    },
    # Phase 28 (28-05, SU-04): свободный текст мимо трёх кнопок развилки резюме больше не
    # записывается молча как резюме (T-28-05-03) — делегат получает эту подсказку вместо тихой
    # записи текста способом, которого не выбирал.
    "reg_resume_fork_pick_hint_text": {
        "type": "text", "group": "reg", "label": "📎 Развилка резюме: подсказка «выбери кнопкой»",
        "prompt": (
            "Текст для делегата, приславшего свободный текст вместо тапа по одной из трёх "
            "кнопок экрана «Как пришлёте резюме?»."
        ),
        "default": "Выбери способ кнопкой выше ⬆️",
    },
    ...
    # Квик 260923-p37 (CITY-REG-CLOSE, D-01/D-05): закрытие регистрации на город по дате —
    # сразу после «🏙 Выбор города: вопрос» (тот же экран «📝 Регистрация», D-01 требует
    # соседства). Все три per_city — дата ставится «для города», как и любой другой
    # переопределяемый ключ; без выбранного города правка меняет ГЛОБАЛЬНОЕ значение
    # (закроет все города сразу — это явно написано в prompt, чтобы менеджер не удивился).
    "city_reg_close_date": {
        "type": "date_only", "group": "reg", "label": "🔒 Город: регистрация закрывается",
        "prompt": (
            "Дата закрытия регистрации на город в формате ДД.ММ.ГГГГ, например 01.10.2026.\n\n"
            "С 00:00 этого дня по Москве город пропадает из выбора при /start, а старые ссылки "
            "на него предлагают другие открытые форумы — уже зарегистрированных делегатов это "
            "не касается.\n\n"
            "Дату ставят ДЛЯ КОНКРЕТНОГО ГОРОДА через «Для города…» — без выбранного города "
            "дата закроет ВСЕ города сразу. «-» убирает дату (город снова открыт)."
        ),
        "default": None,
        "per_city": True,
    },
    "city_reg_closed_text": {
        "type": "text", "group": "reg", "label": "🔒 Город закрыт: текст",
        "prompt": (
            "Что видит человек по ссылке на закрытый (по дате или выключенный) город. "
            "«{city}» — подставится название этого города. Под текстом бот сам добавит кнопки "
            "открытых городов, отдельно рисовать их не нужно."
        ),
        "default": "Регистрация на {city} закрыта. Можно зарегистрироваться на другой форум:",
        "per_city": True,
    },
    "city_reg_all_closed_text": {
        "type": "text", "group": "reg", "label": "🔒 Все города закрыты: текст",
        "prompt": (
            "Что видит человек, если открытых городов не осталось вовсе (все закрыты по дате "
            "или выключены) — кнопок под этим текстом не будет."
        ),
        "default": "Регистрация закрыта.",
        "per_city": True,
    },
    ...
    # Квик 260919-u7e (находка #3): молчание бота после рестарта посреди анкеты — FSM
    # (MemoryStorage) теряется, а живой reg_drafts остаётся. Текст-«мостик» перед тем же
    # экраном «Продолжить/Заново» (offer_resume), что и /start, когда делегат отвечает боту
    # после рестарта без единого сообщения в ответ.
    "reg_resume_after_restart_text": {
        "type": "text", "group": "reg", "label": "🔄 После рестарта бота — анкета сохранена",
        "prompt": (
            "Показывается делегату, если он написал боту (или нажал старую кнопку анкеты) "
            "после того, как бот перезапускался посреди его регистрации — прогресс не потерян, "
            "ниже сразу идёт обычный экран «Продолжить/Заново»."
        ),
        "default": "🔄 Бот перезапускался, но твоя анкета сохранена — продолжим?",
    },
    ...
    "reg_form_resume_upload_button_text": {
        "type": "text", "group": "reg", "label": "📎 Кнопка «Загрузить файл» (резюме)",
        "prompt": (
            "Подпись рядом с иконкой загрузки на шаге «Резюме» в приложении — живая приёмка "
            "16.09 нашла кнопку без подписи (делегат не понимал, что это загрузка файла)."
        ),
        "default": "Загрузить файл",
    },
    "reg_form_resume_text_button_text": {
        "type": "text", "group": "reg", "label": "✍️ Кнопка «Написать текстом» (резюме)",
        "prompt": (
            "Подпись рядом с иконкой-карандашом на шаге «Резюме» в приложении — переключает "
            "поле на текстовый ответ вместо файла."
        ),
        "default": "Написать текстом",
    },
    ...
    "reg_form_more_questions_text": {
        ...
        "prompt": (
            ...
            "автоматически. Формулировка НЕ должна зависеть от склонения числа (число может "
            "быть любым — 1, 2 или 11) — избегай слова, которое надо склонять по {n}."
        ),
        "default": "Впереди ещё вопросов: {n}",
    },
    ...
    # УАТ 10-11.09 (квик 260911-2kb, пункт 3): пустой ввод на необязательных шагах не проходит
    # validate_answer (нужен буквально "-"), а слово "Пропустить" из UI отправить нечем — делегат
    # в приложении упирался в тупик там, где в чате бота кнопка "Пропустить" есть всегда.
    "reg_form_skip_cta_text": {
        "type": "text", "group": "reg", "label": "⏭ Кнопка «пропустить» в мастере",
        "prompt": (
            "Кнопка пропуска необязательного вопроса в приложении. В чате такая кнопка есть "
            "всегда."
        ),
        "default": "Пропустить",
    },
    ...
    # Quick 260919-mlu: массовый значок «это вкладка бота» — экран «📄 Вкладки таблицы»
    # приписывает его сразу ко всем вкладкам бота кнопкой (settings_ops.plan_prefix_renames),
    # это не переименование по одной. Дефолт — сам значок с пробелом, чтобы «Незавершённые»
    # стало «🤖 Незавершённые», а не «🤖Незавершённые».
    "sheet_tab_bot_prefix": {
        "type": "text", "group": "sheets", "label": "🤖 Префикс вкладок бота",
        "prompt": (
            "Что бот припишет в начало имён своих вкладок, чтобы их было видно среди "
            "ручных. Пробел после значка бот добавит сам.\n\nПример: 🤖 → «🤖 Заявки»."
            "\n\nОставьте пустым — префикс не используется."
        ),
        "default": "🤖 ",
    },
    ...
    # ВСЕХ делегатов при первом /start, независимо от language_code клиента (он ненадёжен —
    # часть делегатов держит клиент не на том языке, каким реально пишет). `option_labels` —
    # тот же приём, что у `game_submit_notify_mode` выше: подписи рендерятся сами, менеджеру
    # код варианта не показывается ни в боте, ни в Mini App.
    "delegate_lang_ask_on_start": {
        "type": "enum", "group": "toggles", "label": "🌐 Спрашивать язык при старте",
        "options": ["off", "on", "everyone"],
        "option_labels": {
            "off": "Не спрашивать",
        ...
        "default": "on",
        "prompt": (
            "Работает, только пока модуль «Английский язык анкеты» включён. Три положения:\n\n"
            "«Не спрашивать» — все начинают анкету на русском, язык можно сменить позже "
            "(переключатель в меню).\n"
            "«Только нерусским клиентам» — вопрос кнопками при первом /start показывается "
            "только делегатам, чей Telegram-клиент не на русском.\n"
            "«Всем при первом /start» — вопрос кнопками показывается КАЖДОМУ новому делегату "
            "при первом /start, даже если клиент на русском (сеть безопасности: клиентский "
            "язык не всегда совпадает с языком, на котором реально пишет человек)."
        ),
    },
    ...
    # трактовки статуса здесь не заводим.
    "reg_edit_policy": {
        "type": "enum", "group": "toggles", "label": "✏️ Правка анкеты делегатом",
        "options": ["always", "until_decision", "never"],
        "option_labels": {
            "always": "всегда можно", "until_decision": "только до решения", "never": "нельзя",
        },
        "prompt": None, "default": "always",
        # Правка 260922-wrg (владелец): по городам — та же лестница cities.get_setting_typed_
        # for_city, что у registration_mode/reg_resume_mode; переключатель в админке (шапка =
        # город) правит только этот город, шапка = «все города»/модуль выключен — общее
        # значение байт-в-байт.
        "per_city": True,
    },
    # Квик 260922-wrg (задача 1): можно ли отклонённому делегату подать анкету ЗАНОВО в этом
    # же сезоне — два положения, дефолт "allow" = сегодняшнее поведение байт-в-байт. Гейтит
    # ТОЛЬКО status == "rejected" ТЕКУЩЕГО сезона (`services/reg_edit_policy.resubmit_gate` +
    # `reg_engine.is_past_season_row`); отклонённый/одобренный ПРОШЛОГО сезона — обычный
    # возвращенец, этот тумблер его не касается ни при каком положении. reg_edit_policy выше
    # не меняется этим ключом.
    "reg_resubmit_after_reject": {
        "type": "enum", "group": "toggles", "label": "🔁 Повторная подача после отказа",
        "options": ["allow", "deny"],
        "option_labels": {"allow": "можно", "deny": "нельзя"},
        "prompt": None, "default": "allow",
        # Правка 260922-wrg (владелец): по городам, тот же приём, что у соседа reg_edit_policy
        # выше — читается через cities.get_setting_typed_for_city(key, event_city делегата).
        "per_city": True,
    },
    ...
    # Phase 30 (30-07, A2-08, 30-UI-SPEC.md § «Экран менеджера "Анкета мероприятия"», артборд
    ...
    "reg_form_v2_master_label_text": {
        "type": "text", "group": "reg_prompts",
        "label": "🆕 Подпись мастер-тумблера (анкета 2.0)",
        "prompt": "Заголовок сегмента «Старая анкета» / «Новая анкета» на экране менеджера.",
        "default": "Новая анкета",
    },
    "reg_form_v2_master_hint_text": {
        "type": "text", "group": "reg_prompts",
        "label": "🆕 Пояснение мастер-тумблера (анкета 2.0)",
        "prompt": "Строка риска под сегментом «Старая/Новая анкета».",
        "default": "Сломается — вернёшь одним нажатием, ответы делегатов останутся на месте.",
    },
    "reg_form_chips_label_text": {
        "type": "text", "group": "reg_prompts", "label": "🧩 Подпись строки «Чипы» (анкета 2.0)",
        "prompt": "Название строки-тумблера «Чипы частых значений» на экране менеджера.",
        "default": "Чипы частых значений",
    },
    "reg_form_chips_hint_text": {
        "type": "text", "group": "reg_prompts", "label": "🧩 Пояснение строки «Чипы» (анкета 2.0)",
        "prompt": "Одна строка «что это даёт» под названием строки-тумблера.",
        "default": "Восемь популярных ответов сразу под вопросом",
    },
    "reg_form_lookup_search_label_text": {
        "type": "text", "group": "reg_prompts",
        "label": "🔎 Подпись строки «Поиск в справочнике» (анкета 2.0)",
        "prompt": "Название строки-тумблера «Поиск в справочнике» на экране менеджера.",
        "default": "Поиск в справочнике",
    },
    "reg_form_lookup_search_hint_text": {
        "type": "text", "group": "reg_prompts",
        "label": "🔎 Пояснение строки «Поиск в справочнике» (анкета 2.0)",
        "prompt": "Одна строка «что это даёт» под названием строки-тумблера.",
        "default": "Города и ВУЗы ищутся по первым буквам",
    },
    "reg_form_edu_card_label_text": {
        "type": "text", "group": "reg_prompts",
        "label": "🎓 Подпись строки «Образование одним экраном» (анкета 2.0)",
        "prompt": "Название строки-тумблера «Образование одним экраном» на экране менеджера.",
        "default": "Образование одним экраном",
    },
    "reg_form_edu_card_hint_text": {
        "type": "text", "group": "reg_prompts",
        "label": "🎓 Пояснение строки «Образование одним экраном» (анкета 2.0)",
        "prompt": "Одна строка «что это даёт» под названием строки-тумблера.",
        "default": "ВУЗ, курс и программа вместо четырёх вопросов",
    },
    "reg_form_repeatable_label_text": {
        "type": "text", "group": "reg_prompts",
        "label": "➕ Подпись строки «Повторяемые блоки» (анкета 2.0)",
        "prompt": "Название строки-тумблера «Повторяемые блоки» на экране менеджера.",
        "default": "Повторяемые блоки",
    },
    "reg_form_repeatable_hint_text": {
        "type": "text", "group": "reg_prompts",
        "label": "➕ Пояснение строки «Повторяемые блоки» (анкета 2.0)",
        "prompt": "Одна строка «что это даёт» под названием строки-тумблера.",
        "default": "«Добавить ещё проект» в опыте и портфолио",
    },
    "reg_form_limit_counter_label_text": {
        "type": "text", "group": "reg_prompts",
        "label": "🔢 Подпись строки «Счётчик лимита» (анкета 2.0)",
        "prompt": "Название строки-тумблера «Счётчик лимита в заголовке» на экране менеджера.",
        "default": "Счётчик лимита в заголовке",
    },
    "reg_form_limit_counter_hint_text": {
        "type": "text", "group": "reg_prompts",
        "label": "🔢 Пояснение строки «Счётчик лимита» (анкета 2.0)",
        "prompt": "Одна строка «что это даёт» под названием строки-тумблера.",
        "default": "«Навыки 0/3» вместо ошибки при четвёртом выборе",
    },
    "reg_form_status_screen_label_text": {
        "type": "text", "group": "reg_prompts",
        "label": "📶 Подпись строки «Экран статуса» (анкета 2.0)",
        "prompt": "Название строки-тумблера «Экран статуса заявки во всю ширину» на экране менеджера.",
        "default": "Экран статуса заявки во всю ширину",
    },
    "reg_form_status_screen_hint_text": {
        "type": "text", "group": "reg_prompts",
        "label": "📶 Пояснение строки «Экран статуса» (анкета 2.0)",
        "prompt": "Одна строка «что это даёт» под названием строки-тумблера.",
        "default": "Вместо строчки «Анкета: одобрена» в профиле",
    },
    "reg_form_header_settings_label_text": {
        "type": "text", "group": "reg_prompts",
        "label": "⚙️ Подпись строки «Настройки в шапке» (анкета 2.0)",
        "prompt": "Название строки-тумблера «Настройки в шапке анкеты» на экране менеджера.",
        "default": "Настройки в шапке анкеты",
    },
    "reg_form_header_settings_hint_text": {
        "type": "text", "group": "reg_prompts",
        "label": "⚙️ Пояснение строки «Настройки в шапке» (анкета 2.0)",
        "prompt": "Одна строка «что это даёт» под названием строки-тумблера.",
        "default": "Тема, язык и вибрация — выбирает делегат",
    },
    "reg_form_haptics_label_text": {
        "type": "text", "group": "reg_prompts",
        "label": "📳 Подпись строки «Вибрация» (анкета 2.0, экран менеджера)",
        "prompt": "Название строки-тумблера «Вибрация» на экране менеджера.",
        "default": "Вибрация",
    },
    "reg_form_haptics_hint_text": {
        "type": "text", "group": "reg_prompts",
        "label": "📳 Пояснение строки «Вибрация» (анкета 2.0, экран менеджера)",
        "prompt": "Одна строка «что это даёт» под названием строки-тумблера.",
        "default": "Короткий отклик на выбор и на ошибку",
    },
    "reg_form_manager_title_text": {
        "type": "text", "group": "reg_prompts", "label": "📝 Заголовок экрана «Анкета мероприятия»",
        "prompt": "Заголовок экрана менеджера «Анкета мероприятия».",
        "default": "Анкета мероприятия",
    },
    "reg_form_manager_subtitle_text": {
        "type": "text", "group": "reg_prompts",
        "label": "📝 Подзаголовок экрана «Анкета мероприятия»",
        "prompt": "Подзаголовок под заголовком экрана менеджера.",
        "default": "Выбери, какой анкетой собираешь заявки. Переключается на ходу, без перезапуска бота.",
    },
    "reg_form_manager_included_eyebrow_text": {
        "type": "text", "group": "reg_prompts",
        "label": "📝 Eyebrow «Что включено» (экран «Анкета мероприятия»)",
        "prompt": "Строка над списком из восьми тумблеров.",
        "default": "Что включено в новой анкете",
    },
    "reg_form_manager_degrade_note_text": {
        "type": "text", "group": "reg_prompts",
        "label": "📝 Общая подсказка деградации (экран «Анкета мероприятия»)",
        "prompt": "Подсказка внизу списка тумблеров — что происходит при выключении элемента.",
        "default": (
            "Выключишь элемент — шаг не сломается, а упростится: вместо чипов останется "
            "поиск, вместо поиска — список, вместо списка — обычный ввод."
        ),
    },
    "reg_form_manager_preview_button_text": {
        "type": "text", "group": "reg_prompts",
        "label": "📝 Кнопка предпросмотра (экран «Анкета мероприятия»)",
        "prompt": "Вторичная кнопка внизу экрана менеджера — открывает анкету глазами делегата.",
        "default": "Посмотреть анкету глазами делегата",
    },
    # Phase 30 (30-07, задача 4, A2-03, 30-CONTEXT.md § «Решения оркестратора», п.2): три
    # атрибута списка-справочника (чипы/поиск/свой вариант) — ключи-спутники `university_
    # options`/`city_options` (существующие списки `type: "list"`), НЕ десятый тумблер группы
    ...
    "university_options_chips_enabled": {
        "type": "enum", "group": "toggles", "label": "🏫 Чипы у списка ВУЗов (анкета 2.0)",
        "options": ["on", "off"], "default": "on",
        "prompt": "Топ-8 частых ВУЗов чипами над полем поиска (только в новой анкете).",
    },
    "university_options_search_enabled": {
        "type": "enum", "group": "toggles", "label": "🏫 Поиск у списка ВУЗов (анкета 2.0)",
        "options": ["on", "off"], "default": "on",
        "prompt": "Поиск по первым буквам вместо статичного списка (только в новой анкете).",
    },
    "university_options_other_allowed": {
        "type": "enum", "group": "toggles",
        "label": "🏫 Разрешить свой вариант ВУЗа (анкета 2.0)",
        "options": ["on", "off"], "default": "on",
        "prompt": (
            "Выключено — делегат обязан выбрать ВУЗ из справочника, свободный текст "
            "заблокирован (только в новой анкете)."
        ),
    },
    "city_options_chips_enabled": {
        "type": "enum", "group": "toggles", "label": "🏙 Чипы у списка городов (анкета 2.0)",
        "options": ["on", "off"], "default": "on",
        "prompt": "Топ-8 частых городов чипами над полем поиска (только в новой анкете).",
    },
    "city_options_search_enabled": {
        "type": "enum", "group": "toggles", "label": "🏙 Поиск у списка городов (анкета 2.0)",
        "options": ["on", "off"], "default": "on",
        "prompt": "Поиск по первым буквам вместо статичного списка (только в новой анкете).",
    },
    "city_options_other_allowed": {
        "type": "enum", "group": "toggles",
        "label": "🏙 Разрешить свой вариант города (анкета 2.0)",
        "options": ["on", "off"], "default": "on",
        "prompt": (
            "Выключено — делегат обязан выбрать город из справочника, свободный текст "
            "заблокирован (только в новой анкете)."
        ),
    },
    ...
    "reg_form_selection_visible_note_text": {
        ...
        "prompt": (
            "Строка под плитками select в приложении — тап по плитке сразу отправляет ответ "
            "и переводит на следующий вопрос (автопереход, приёмка 16.09)."
        ),
        "default": "Нажми на вариант — сразу перейдём к следующему вопросу.",
    },
    ...
    # Phase 30 (30-06, A2-05): два под-вопроса на блок в чате — «Название проекта?» →
    # «Опиши коротко» (30-CONTEXT.md § «Решения по итогам 30-RESEARCH.md», п.1). Ключей под эти
    # два вопроса не заводили планы 30-01/30-04 (там только «Добавить ещё?»/кнопки) — заводим
    # здесь, вместе с чат-хендлером, который их первым читает (Rule 2).
    "reg_repeatable_chat_title_prompt_text": {
        "type": "text", "group": "reg_prompts", "label": "➕ «Название» (repeatable, чат)",
        "prompt": "Первый под-вопрос блока в чате — {noun} задаёт конкретный шаг.",
        "default": "Как называется этот {noun}?",
    },
    "reg_repeatable_chat_description_prompt_text": {
        "type": "text", "group": "reg_prompts", "label": "➕ «Опиши коротко» (repeatable, чат)",
        "prompt": "Второй под-вопрос блока в чате — свободное описание.",
        "default": "Опиши коротко.",
    },
    ...
    # «💬 Чат» снесён целиком, привязка чата теперь идёт через личку (`handlers/group_chat.py`).
    ...
    # Phase 32 (32-06, D-36): третий источник монет рядом с manual/task — авто-начисление
    # амбассадору за одобренного приглашённого (claim_referral_credit, план 32-01).
    "balance_source_referral_label": {
        "type": "text", "group": "game", "label": "🪙 Баланс: подпись источника «приглашённый»",
        "prompt": "Подпись источника операции в истории баланса, когда монеты начислены автоматически за одобренного приглашённого амбассадором.",
        "default": "за приглашённого",
    },
    ...
    # ── Phase 32 (32-02, D-21/D-35/D-18/D-29): амбассадорский слой — числа и тумблеры.
    # Дефолты 0/0 обязательны (D-21/D-35): на уже работающих событиях (РилТолк, СкиллАп, прод
    # Юлида) обновление не должно молча начислить ни одного балла и не должно молча оштрафовать
    # ни одну сдачу — менеджер задаёт числа осознанно. Рекомендованные значения (30%, «≈задание
    # средней категории») — только в тексте подсказки, не в default.
    "ambassador_referral_coins": {
        "type": "int", "group": "game", "label": "🤝 Баллы за приглашённого",
        "prompt": (
            "За КАЖДУЮ одобренную заявку приглашённого амбассадор получает столько баллов. "
            "Начисление разовое и не снимается, если приглашённому потом откажут.\n\n"
            "Примеры:\n0 = не начислять (по умолчанию)\n50 = 50 баллов за одобренного\n\n"
            "Рекомендация — примерно столько же, сколько стоит задание средней категории.\n\n"
            "Меняется на лету, перезапуск не нужен."
        ),
        "default": 0,
    },
    "game_late_penalty_percent": {
        "type": "int", "group": "game", "label": "⏰ Штраф за сдачу после дедлайна, %",
        "prompt": (
            "На сколько процентов уменьшается награда, если сдача пришла после дедлайна. "
            "Штраф фиксированный и не зависит от того, насколько поздно сдали.\n\n"
            "Примеры:\n0 = без штрафа (по умолчанию)\n30 = обычная практика (за задание на "
            "100 баллов делегат получит 70)\n\n"
            "Менеджер видит итоговую сумму на карточке проверки до того, как нажмёт "
            "«Одобрить».\n\nМеняется на лету, перезапуск не нужен."
        ),
        "default": 0,
    },
    "wave_prize_places": {
        "type": "int", "group": "game", "label": "🏅 Призовых мест в волне",
        "prompt": (
            "Сколько первых мест считаются призовыми при объявлении итогов волны. У "
            "конкретной волны можно задать своё число.\n\nПримеры:\n3 = призовые «топ-3» "
            "(по умолчанию)\n5 = пять призовых мест\n\n"
            "Обычно призовых мест 10–20% от активных амбассадоров — если сделать их слишком "
            "много, победа перестаёт что-то значить.\n\nМеняется на лету, перезапуск не нужен."
        ),
        "default": 3,
    },
    ...
    # который подставляется в алерт кнопки.
    "wave_rating_show_names": {
        "type": "enum", "group": "game", "label": "🏅 Показывать имена в рейтинге волны",
        "options": ["on", "off"],
        "prompt": (
            "Включено — амбассадор видит топ с именами и своё место. Выключено — только "
            "своё место и отрыв от призового.\n\n"
            "Выключайте для маленьких волн или если участники жалуются на давление рейтинга."
        ),
        "default": "on",
    },
    ...
    "wave_start_message_text": {
        "type": "text", "group": "game", "label": "🌊 Старт волны: текст",
        "prompt": (
            "ЛС каждому амбассадору при старте волны.\n\nПодставляются:\n{wave} — номер "
            "волны (просто число, например 2 — слово «Волна» уже есть в тексте по умолчанию)"
            "\n{ends} — дата окончания\n{intro} — вводный текст волны, если "
            "менеджер его задал (иначе пустая строка)\n{tasks} — список заданий волны "
            "(каждое отдельной строкой)\n\nПлейсхолдеры не убирать. Поддерживается HTML."
        ),
        "default": (
            "Волна {wave} началась и продлится до {ends}.\n{intro}\n\nЗадания волны:\n{tasks}"
            "\n\nСдать можно и позже дедлайна — баллов будет меньше."
        ),
    },
    "wave_start_button_text": {
        "type": "text", "group": "game", "label": "🌊 Старт волны: кнопка",
        "prompt": "Подпись кнопки под сообщением о старте волны — ведёт в список заданий.",
        "default": "Открыть задания",
    },
    "wave_deadline_reminder_text": {
        "type": "text", "group": "game", "label": "🌊 Напоминание о дедлайне",
        "prompt": (
            "Уходит за сутки, только тем, кто ещё не сдал задание.\n\nПодставляются:\n"
            "{task} — название задания\n{coins} — сколько баллов задание стоит\n"
            "{deadline} — дата и время дедлайна\n\nПлейсхолдеры не убирать. Поддерживается HTML."
        ),
        "default": (
            "Скоро дедлайн ({deadline}), а у тебя ещё не сдано: «{task}» ({coins} баллов). "
            "Успеешь — получишь баллы полностью, после дедлайна — со штрафом."
        ),
    },
    "wave_results_announce_text": {
        "type": "text", "group": "game", "label": "🌊 Итоги волны: объявление всем",
        "prompt": (
            "Уходит всем амбассадорам волны после объявления итогов.\n\nПодставляются:\n"
            "{wave} — номер волны (просто число, например 2 — слово «Волна» уже есть в "
            "тексте по умолчанию)\n{winners} — призёры волны (имена)\n"
            "{place} — место ЭТОГО амбассадора\n{total} — сколько всего было активных\n"
            "{points} — сколько баллов набрал этот амбассадор за волну\n\n"
            "Плейсхолдеры не убирать. Поддерживается HTML."
        ),
        "default": (
            "Волна {wave} завершена. Призёры: {winners} — поздравляем!\n"
            "Твоё место — {place}-е из {total}, {points} баллов за волну."
        ),
    },
    "wave_results_winner_text": {
        "type": "text", "group": "game", "label": "🌊 Итоги волны: поздравление призёру",
        "prompt": (
            "Отдельное ЛС только призёрам (по числу призовых мест волны — при равенстве "
            "баллов на границе призёров может оказаться больше, чем самих мест).\n\n"
            "Подставляются:\n{wave} — номер волны (просто число, например 2 — слово "
            "«Волна» уже есть в тексте по умолчанию)\n{place} — место призёра\n{points} — "
            "баллы за волну\n{prize} — текст приза (ключ «Приз: текст в итогах», ниже)\n\n"
            "Плейсхолдеры не убирать. Поддерживается HTML."
        ),
        "default": (
            "Ты среди призёров волны {wave} — {place}-е место, {points} баллов!\n{prize}"
            "\nСпасибо за работу в этой волне."
        ),
    },
    "wave_results_prize_text": {
        "type": "text", "group": "game", "label": "🌊 Приз: текст в итогах",
        "prompt": (
            "Сам текст приза, который подставляется в сообщение призёру ({prize} выше) — "
            "ТОЛЬКО текст, бот не начисляет за место ни коины, ни что-либо ещё сам: если "
            "приз материальный, вручает его менеджер лично, этот текст просто анонс."
        ),
        "default": "Твой приз найдёт тебя отдельно — считай это счастливым билетом 🎟",
    },
    "wave_rating_header_text": {
        "type": "text", "group": "game", "label": "🌊 Рейтинг волны: заголовок",
        "prompt": "Первая строка экрана рейтинга ТЕКУЩЕЙ волны. Поддерживается HTML.",
        "default": "🏅 <b>Рейтинг волны</b>",
    },
    "wave_rating_own_line_text": {
        "type": "text", "group": "game", "label": "🌊 Рейтинг волны: строка «твоё место»",
        "prompt": (
            "Строка про самого амбассадора на экране рейтинга волны — показывается всегда, "
            "даже когда «Показывать имена в рейтинге волны» выключено.\n\nПодставляются:\n"
            "{rank} — его место\n{total} — сколько всего активных в волне\n"
            "{place} — призовое место, до которого считается отрыв\n"
            "{gap} — сколько баллов не хватает до призового места (0, если уже в призах)\n\n"
            "Плейсхолдеры не убирать."
        ),
        "default": "Ты {rank}-й из {total}. До призового места ({place}-е) — {gap} баллов.",
    },
    "wave_rating_closed_text": {
        "type": "text", "group": "game", "label": "🌊 Рейтинг волны: волна не идёт",
        "prompt": "Что видит амбассадор на экране рейтинга волны, пока активной волны нет.",
        "default": "Сейчас нет активной волны амбассадоров — рейтинг появится, когда волна начнётся.",
    },
    "ambassador_block_header_text": {
        "type": "text", "group": "game", "label": "🌊 Амбассадорский блок: заголовок",
        "prompt": "Заголовок раздела заданий амбассадоров (отдельно от обычных заданий волны).",
        "default": "Задания амбассадоров",
    },
    "game_task_no_deadline_text": {
        "type": "text", "group": "game", "label": "🌊 Карточка задания: без срока",
        "prompt": "Что показывается вместо даты у задания без собственного дедлайна.",
        "default": "без срока",
    },
    "game_task_penalty_hint_text": {
        "type": "text", "group": "game", "label": "🌊 Карточка задания: строка штрафа",
        "prompt": (
            "Строка на карточке задания про штраф за просрочку — только у заданий, у которых "
            "включён штраф.\n\nПодставляются:\n{deadline} — дата и время дедлайна\n"
            "{penalized} — сумма баллов ПОСЛЕ штрафа\n{coins} — полная сумма баллов задания\n\n"
            "Плейсхолдеры не убирать."
        ),
        "default": (
            "После дедлайна ({deadline}) — {penalized} баллов вместо {coins}. Штраф "
            "фиксированный, не зависит от того, насколько поздно."
        ),
    },
    "ambassador_path_prompt_text": {
        "type": "text", "group": "game", "label": "🌊 Путь амбассадора: вопрос",
        "prompt": "Вопрос, каким путём амбассадор хочет приносить пользу (выбор кнопками).",
        "default": "Как тебе больше нравится помогать как амбассадору?",
    },
    "ambassador_path_label_invite": {
        "type": "text", "group": "game", "label": "🌊 Путь: «Зову людей»",
        "prompt": "Подпись кнопки-варианта «зову людей на мероприятие».",
        "default": "Зову людей",
    },
    "ambassador_path_label_content": {
        "type": "text", "group": "game", "label": "🌊 Путь: «Делаю контент»",
        "prompt": "Подпись кнопки-варианта «делаю контент для соцсетей».",
        "default": "Делаю контент",
    },
    "ambassador_path_label_none": {
        "type": "text", "group": "game", "label": "🌊 Путь: «Пока не выбрал»",
        "prompt": "Подпись кнопки-варианта «пока не определился с путём».",
        "default": "Пока не выбрал",
    },
    "ambassador_leave_button_text": {
        "type": "text", "group": "game", "label": "🌊 Кнопка «Перестать быть амбассадором»",
        "prompt": "Подпись кнопки выхода из амбассадорского блока.",
        "default": "Перестать быть амбассадором",
    },
    "ambassador_leave_confirm_text": {
        "type": "text", "group": "game", "label": "🌊 Выход из амбассадоров: подтверждение",
        "prompt": (
            "Экран подтверждения перед выходом — прямо перечисляет, что пропадёт, а что "
            "останется (CLAUDE.md: разрушительная операция — с подтверждением)."
        ),
        "default": (
            "Точно хочешь перестать быть амбассадором?\n\nРассылки волны прекратятся, из "
            "рейтинга текущей волны ты исчезнешь, а уже набранные баллы останутся."
        ),
    },
    "ambassador_leave_done_text": {
        "type": "text", "group": "game", "label": "🌊 Выход из амбассадоров: готово",
    ...
    "wave_end_manager_text": {
        "type": "text", "group": "game", "label": "🌊 Конец волны: сообщение менеджеру",
        "prompt": (
            "Уходит менеджеру, когда волна закончилась — до объявления итогов делегатам.\n\n"
            "Подставляются:\n{wave} — номер волны (просто число, например 2 — слово «Волна» "
            "уже есть в тексте по умолчанию)\n{top} — предварительный топ по "
            "уже проверенным сдачам\n{pending} — сколько сдач ещё не проверено\n\n"
            "Плейсхолдеры не убирать. Менеджеру, не делегату — латиница брендов тут не "
            "запрещена, но и не нужна. Поддерживается HTML."
        ),
        "default": (
            "Волна {wave} закончилась. Топ: {top}. На проверке ещё {pending} сдач — итог "
            "может измениться, если проверить их сейчас."
        ),
    },
    ...
    "proxy_switch_dwell_seconds": {
        ...
        "prompt": (
            "Сколько секунд бот держится на канале, ПРЕЖДЕ чем признать его мёртвым и уйти "
            "дальше — но только если канал уже доказал, что работает (был хотя бы один "
            "успешный запрос), или это основной канал. Резервный канал, который ещё ни разу "
            "не сработал, бот бросает сразу при первой же ошибке — держаться за непроверенный "
            "резерв незачем, это только добавляет тишины для делегатов.\n\n"
            "В СЕКУНДАХ. Примеры:\n30 = полминуты (по умолчанию)\n60 = минута\n\n"
            "⚠️ Применяется только после перезапуска бота."
        ),
        "default": 30,
    },
    ...
    "incomplete_sync_hours": {
        # Квик 260923 (AUTOREJ-REPORT, D-F): ключ и default НЕ меняются — вкладка автоотказов
        # (auto_reject_sheet_tab ниже) обновляется в ТОЙ ЖЕ каденции, отдельного интервала под
        # неё не заводим. Подпись расширена на обе вкладки, чтобы менеджер не искал вторую
        # настройку, которой нет.
        "type": "int", "group": "system", "label": "⏱ Вкладки «Незавершённые» и автоотказов: автообновление",
        "prompt": (
            "Как часто бот сам обновляет вкладку незавершённых регистраций и вкладку "
            "автоотказов (если задана) в таблице.\n\n"
            "В ЧАСАХ. Примеры:\n2 = раз в два часа (по умолчанию)\n1 = каждый час\n\n"
            "⚠️ Применяется только после перезапуска бота."
        ),
        "default": 2,
    },
    # Quick 260907-4ai (P0 SkillUp5): если облако было недоступно в момент отправки анкеты,
    # бот сам повторяет выгрузку файла — эта настройка задаёт частоту проверки очереди.
    "resume_retry_minutes": {
        "type": "int", "group": "system", "label": "☁️ Повтор выгрузки резюме в облако",
        "prompt": (
            "Если облако было недоступно в момент отправки анкеты, бот сам повторит "
            "выгрузку файла — здесь настраивается, как часто он проверяет очередь.\n\n"
            "В МИНУТАХ. Примеры:\n10 = каждые 10 минут (по умолчанию)\n30 = реже\n\n"
            "⚠️ Применяется только после перезапуска бота."
        ),
        "default": 10,
    },
    # Квик 260911-mx6: сеялка состояний приёмки — тестер одной командой /uat ставит СВОЙ
    # аккаунт в нужное состояние без ручного заполнения анкеты. Дефолт тумблера ВЫКЛ
    # обязателен: операция необратимо стирает данные, менеджер включает её осознанно и
    # только на время приёмки.
    "uat_seed_enabled": {
        "type": "enum", "group": "system", "label": "🧪 Сеялка состояний /uat",
        "options": ["on", "off"], "prompt": None, "default": "off",
    },
    "uat_seed_testers": {
        "type": "list", "group": "system", "label": "🧪 Кто может пользоваться /uat",
        "prompt": (
            "Кто может пользоваться командой /uat — по одному на строку ИЛИ через «;» "
            "(на телефоне Enter отправляет сообщение раньше, чем допишете список).\n\n"
            "Можно «@ник», можно числовой id, например:\n@qleafye; 873278697\n\n"
            "⚠️ Вне приёмки список стоит очистить, а тумблер «Сеялка состояний /uat» "
            "выключить."
        ),
        "default": None,
    },
    ...
    "chat_refresh_minutes": {
        "type": "int", "group": "system", "label": "⏱ Чат делегатов: как часто сверять состав",
        "prompt": (
            "Как часто бот сверяет состав чата делегатов с Telegram (кто реально состоит, "
            "кто вышел) — отдельно от событий вступления/выхода, которые бот ловит сразу.\n\n"
            "В МИНУТАХ. Примеры:\n360 = раз в 6 часов (по умолчанию)\n60 = каждый час\n\n"
            "⚠️ Применяется только после перезапуска бота."
        ),
        "default": 360,
    },
    "delegate_chat_id": {
        "type": "text", "group": "system", "label": "💬 Чат делегатов (id)",
        "prompt": (
            "Значение пишет сам бот, когда его добавляют в группу администратором — "
            "руками менять не нужно."
        ),
        "default": "",
    },
    "delegate_chat_title": {
        "type": "text", "group": "system", "label": "💬 Чат делегатов (название)",
        "prompt": (
            "Значение пишет сам бот, когда его добавляют в группу администратором — "
            "руками менять не нужно."
        ),
    ...
    # Quick 260911-5ij (W2): пять текстов состояний — доставляются во фронт-оболочку через
    # data-screen-texts (page.py::SCREEN_TEXT_KEYS), а не хардкодятся в JS. Первые два красят
    # общую обёртку первичной загрузки экрана (guardedRender в ui.js), последние три — гейт
    # экрана сдачи по task.can_submit.
    "miniapp_load_error_text": {
        "type": "text", "group": "miniapp", "label": "⚠️ Текст «не удалось загрузить»",
        "prompt": (
            "Текст, когда экран приложения не смог загрузиться (обрыв связи или ошибка "
            "сервера) — виден делегату вместо пустого экрана."
        ),
        "default": "Не удалось загрузить данные. Проверьте связь и нажмите «Повторить».",
    },
    "miniapp_retry_button": {
        "type": "text", "group": "miniapp", "label": "🔁 Кнопка «Повторить»",
        "prompt": "Подпись кнопки повторной загрузки экрана после ошибки.",
        "default": "Повторить",
    },
    # Квик 12.09 (UI-аудит, пункт 3): шестой ключ SCREEN_TEXT_KEYS — обрыв сети на любом шаге
    # анкеты (не только на первичной загрузке экрана, как miniapp_load_error_text выше) раньше
    # давал пустую плашку (12 мест в screens/form.js звали errorText(err, "")).
    "miniapp_network_error_text": {
        "type": "text", "group": "miniapp", "label": "📡 Текст «нет связи» в приложении",
        "prompt": (
            "Текст, когда запрос из приложения не дошёл до сервера (обрыв сети на любом шаге "
            "анкеты) — виден делегату вместо пустой плашки."
        ),
        "default": "Не дошло до сервера. Проверьте связь и попробуйте ещё раз.",
    },
    "miniapp_submit_pending_text": {
        "type": "text", "group": "miniapp", "label": "🕓 Текст «сдача уже на проверке»",
        "prompt": (
            "Текст на экране сдачи, если по заданию уже есть ответ на проверке — форма ответа "
            "не показывается."
        ),
        "default": "Задание уже отправлено и ждёт проверки — решение придёт в бот.",
    },
    "miniapp_submit_approved_text": {
        "type": "text", "group": "miniapp", "label": "✅ Текст «задание уже принято»",
        "prompt": "Текст на экране сдачи, если задание уже принято — форма ответа не показывается.",
        "default": "Это задание уже принято.",
    },
    "miniapp_submit_limit_text": {
        "type": "text", "group": "miniapp", "label": "🚫 Текст «попытки закончились»",
        "prompt": (
            "Текст на экране сдачи, если попытки по заданию закончились (лимит пересдач "
            "исчерпан) — форма ответа не показывается."
        ),
        "default": "Попытки по этому заданию закончились. Напишите менеджеру, если это ошибка.",
    },
    ...
    # Приёмка 17.09 (п.1): постоянное место реф-ссылки в хабе делегата (`GET /app/api/hub` ->
    # `referral`) — до этого ссылку можно было увидеть только один раз, на терминальном
    # экране анкеты («Хочу свою ссылку»), или в чате кнопкой «🔗 Моя реферальная ссылка»
    # (`keyboards/builders.py::MENU_BUTTONS`). Правило видимости — ТО ЖЕ, что у этой кнопки:
    # тумблер `menu_referral` (per_city) + одобренная заявка (сам маршрут уже требует
    # `delegate_gate`, аналог `ensure_registered`). Заголовок блока и подпись кнопки
    # «Скопировать» переиспользуют существующие ключи финального экрана анкеты
    # (`miniapp_form_ambassador_copy_button_text`/`_copied_toast_text`) — второй набор тех
    # же по смыслу текстов не заводится.
    "miniapp_hub_referral_label_text": {
        "type": "text", "group": "miniapp", "label": "🔗 Подпись раздела «Моя реферальная ссылка»",
        "prompt": (
            "Подпись постоянного места реф-ссылки на первом экране приложения — то же слово, "
            "что видит делегат в чате на кнопке «🔗 Моя реферальная ссылка»."
        ),
        "default": "🔗 Моя реферальная ссылка",
    },
    "miniapp_hub_referral_invites_text": {
        "type": "text", "group": "miniapp", "label": "🔗 Число приглашённых рядом со ссылкой",
        "prompt": (
            "Строка под ссылкой — сколько человек уже пришло по ней (второй тумблер "
            "«👥 Мои приглашённые» должен быть включён, иначе строки не будет). Обязательна "
            "подстановка {count} — вместо неё встанет число."
        ),
        "default": "Приглашено по твоей ссылке: {count}",
    },
    ...
    # отдельная подпись, «Открыть резюме» неточно называет чужую ссылку-профиль.
    "miniapp_applications_resume_open_link": {
        "type": "text", "group": "miniapp", "label": "🗂 Кнопка «Открыть ссылку»",
        "prompt": "Подпись кнопки, которая открывает ссылку резюме (делегат дал ссылку вместо файла).",
    ...
    # Правила автоотказа: чип «только помеченные правилами» (D-20) — точная копия соседа выше.
    "miniapp_applications_filter_flagged": {
        "type": "text", "group": "miniapp", "label": "🗂 Фильтр «Помечены правилом»",
        "prompt": "Подпись чипа-фильтра «только заявки, помеченные правилом автоотказа».",
        "default": "Помечены правилом",
    },
    ...
    "miniapp_settings_search_placeholder_text": {
        ...
        "prompt": (
            "Подсказка в пустой строке поиска настроек. Держите короткой — поле шириной "
            "с экран телефона обрезает длинный текст на середине слова."
        ),
        # Квик 12.09 (UI-аудит, пункт 10): прежний дефолт («Найти настройку — например,
        # «приветствие» или «дедлайн»») резался на 390px-экране на середине слова.
        "default": "Найти настройку…",
    },
    ...
    # Квик 12.09 (UI-аудит, пункт 4): счётчик в шапке карточки группы настроек — был голым
    # числом (String(group.items.length)), 22-UI-SPEC §Typography требует «N из M». «{total}» —
    # тумблеры раздела + items всех групп раздела (тумблеры подняты НАД карточками групп).
    ...
    # Phase 28 (28-08, SU-08, A5): чекбокс-пикер `list` + `options_from_step` — подпись,
    # которую менеджер выбрал РАНЬШЕ, но которой уже нет в текущем списке вариантов вопроса
    # (28-UI-SPEC §8: «варианта больше нет», отдельной строкой-чипом с крестиком, значение не
    # теряется молча). Один текст на все ключи `options_from_step` — то же, что
    # `miniapp_settings_stale_badge_text` выше делает для другого вида «устарело».
    "miniapp_settings_option_gone_text": {
        "type": "text", "group": "miniapp", "label": "⚠️ Пропавший вариант списка",
        "prompt": (
            "Пометка у сохранённого варианта, который пропал из текущего списка вопроса "
            "анкеты (чекбокс-пикеры вроде «🧮 Правила балла»)."
        ),
        "default": "Варианта больше нет",
    },
    ...
    # Phase 28 (28-10, SU-11, 28-UI-SPEC.md «Copywriting Contract»): подтверждение
    # применения пресета «🎓 Форум СкиллАп» — своё, отдельное от общего
    # miniapp_settings_confirm_event_type_text (dangerous_confirm_key различает направление
    # по next_value == "skillup"), потому что пресет меняет не два модуля, а десятки
    # тумблеров вопросов/скоринга/резюме/догонялки разом — менеджер должен увидеть это
    # человеческим языком (D-01), а не общей фразой про «Оплату»/«Согласия».
    "skillup_preset_confirm_text": {
        "type": "text", "group": "miniapp", "label": "⚠️ Подтверждение: пресет «СкиллАп»",
        "prompt": (
            "Текст подтверждения применения пресета «🎓 Форум СкиллАп» — что именно "
            "включится, показывается перед применением и в боте, и в веб-настройках."
        ),
        "default": (
            "Пресет «СкиллАп» включит анкету направлений/стека/резюме, пропуск вопроса "
            "«Откуда узнал» по реф-ссылке, догонялку и другие настройки события. Применить?"
        ),
    },
    ...
    # Приёмка 17.09 (п.2): делегат тапал «Хочу свою ссылку» и видел голый URL без контекста —
    # непонятно, что с ним делать, если не скопировать прямо сейчас. Пояснение — ГДЕ ссылку
    # реально можно найти потом: кнопка меню «🔗 Моя реферальная ссылка» в чате
    # (`handlers/user_actions.py::my_referral_link`, работает всегда, без гейта по статусу
    # заявки) И постоянное место в приложении (`GET /app/api/hub` -> `referral`, доступно
    # ТОЛЬКО одобренному делегату — `delegate_gate`), подпись раздела там — `{section}`,
    # плейсхолдер подставляется тем же ключом `miniapp_hub_referral_label_text`, второй
    # литерал названия раздела не заводим. Показывается на обеих поверхностях (чат
    # `reg_ambassador.regamb_want`, приложение `miniapp/routers/form.py::draft_ambassador`)
    # сразу после самой ссылки, БЕЗ гейта по одобрению у себя (та же логика, что у ссылки
    # выше) — оговорка «после одобрения» относится к разделу приложения, не к самому тексту.
    "miniapp_form_ambassador_link_note_text": {
        "type": "text", "group": "miniapp", "label": "🎁 Пояснение под ссылкой",
        "prompt": (
            "Короткая строка сразу под ссылкой — где её найти потом (бот и приложение). "
            "Подставляется:\n{section} — подпись раздела приложения (та же, что на плите "
            "«🔗 Моя реферальная ссылка»)\n\nПлейсхолдер не убирать — иначе фраза про "
            "приложение не соберётся."
        ),
        "default": (
            "Эту ссылку всегда можно найти по кнопке «{section}» — в чате с ботом и в "
            "приложении (в приложении — после одобрения заявки)."
        ),
        "per_city": True,
    },
    # Квик 12.09 (UI-аудит, пункт 2): кнопка выхода на терминальном экране «Заявка принята»
    # сегодня — одна иконка без подписи и без aria-label (accessibility BLOCKER). Текст —
    # служебный интерфейсный, без per_city: экран один для всех городов.
    "miniapp_form_complete_home_cta_text": {
        "type": "text", "group": "miniapp", "label": "🏁 Кнопка выхода на экране «Заявка принята»",
        "prompt": "Подпись кнопки выхода на хаб на терминальном экране анкеты (только приложение).",
        "default": "На главную",
    },

    # Квик 260915-4mu: мастер первой настройки (`miniapp/setup_wizard.py`) сам ничего в
    # bot_settings не хранит — единственное исключение прямо в решении владельца (план,
    # п.6): тумблер «скрыть плитку» на хабе менеджера. Own group "miniapp" (самоописательная
    # подпись, синонимы не нужны — tests/test_settings_synonyms.py), пишется тем же
    # settings/batch, что и остальные ключи, отдельного пути записи мастер не заводит.
    "setup_wizard_dismissed": {
        "type": "enum", "group": "miniapp", "options": ["on", "off"],
        "label": "🚀 Мастер первой настройки: скрыть плитку",
        "prompt": (
            "«включено» — плитка «🚀 Первая настройка» в приложении больше не показывается. "
            "Сам мастер остаётся доступен по прямой ссылке из хаба."
        ),
        "default": "off",
    },
    ...
    # Phase 28 (28-07, SU-08, A-04, СкиллАп 5): автоскоринг заявки — веса ТЗ §3.6 жёсткие
    # (2/2/1/1/1, зашиты в reg_engine.compute_score), а множества/пороги правит менеджер.
    # `options_from_step` — новый атрибут ключа (читают пикеры плана 28-08: бот-экран по
    # образцу admin_modcard.py и веб-чекбоксы form.js); сам движок использует значение как
    # обычный список подписей через reg_engine.option_list_for. Пустой список = «этот пункт
    # никому не начисляется» (D-06) — до 28-08 менеджер правит эти четыре ключа тем же общим
    # текстовым редактором списков, что и *_options (список каждый пункт с новой строки).
    "score_it_fields": {
        "type": "list", "group": "apps", "label": "🧮 Балл: IT-направления",
        "options_from_step": "study_field",
        "prompt": (
            "Направления обучения (из списка вопроса «Направление обучения»), которые "
            "считаются IT — каждое с новой строки. Пусто = ни одно направление не даёт балл "
            "этим пунктом (стек всё ещё может дать его отдельно)."
        ),
        "default": None,
    },
    "score_senior_statuses": {
        "type": "list", "group": "apps", "label": "🧮 Балл: старшие статусы образования",
        "options_from_step": "education_status",
        "prompt": (
            "Статусы образования (из списка вопроса «Образование»), которые считаются "
            "«старшими» — каждый с новой строки. Пусто = ни один статус не даёт балл этим "
            "пунктом (курс всё ещё может дать его отдельно)."
        ),
        "default": None,
    },
    "score_readiness_counts": {
        "type": "list", "group": "apps", "label": "🧮 Балл: какая готовность засчитывается",
        "options_from_step": "readiness",
        "prompt": (
            "Варианты готовности выйти на работу (из списка вопроса «Готовность»), за "
            "которые начисляется балл — каждый с новой строки. Пусто = баллов за готовность "
            "не начисляется."
        ),
        "default": None,
    },
    "score_experience_counts": {
        "type": "list", "group": "apps", "label": "🧮 Балл: какой опыт засчитывается",
        "options_from_step": "experience",
        "prompt": (
            "Варианты опыта работы (из списка вопроса «Опыт»), за которые начисляется балл — "
            "каждый с новой строки. Пусто = баллов за опыт не начисляется."
        ),
        "default": None,
    },
    "score_course_from": {
        "type": "int", "group": "apps", "label": "🧮 Балл: курс от",
        "prompt": "С какого курса засчитывать балл за старшинство. Например: 3.",
        "default": 3,
    },
    "score_stack_from": {
        "type": "int", "group": "apps", "label": "🧮 Балл: сколько пунктов стека засчитывать",
        "prompt": (
            "Сколько пунктов «Стека» нужно отметить делегату, чтобы засчитать направление "
            "как IT. Например: 2."
        ),
        "default": 2,
    },
    ...
    # Phase 31 (31-03, D-15): ГЛАВНЫЙ рубильник правил автоотказа — одной кнопкой выключает
    # оценку ВСЕХ правил сразу (состояние каждого отдельного правила при этом сохраняется).
    # Default НАРОЧНО "off" — на РилТолке и СкиллАпе, где менеджер правила не завёл и модуль
    # не включал, ничего не меняется (та же конвенция, что у reg_scoring_enabled выше). Кнопка
    ...
    # Квик 260923 (AUTOREJ-REPORT, D-E/D-I): имя вкладки таблицы, куда бот сам выгружает всех,
    # кого отсеяли правила автоотказа — полная перезапись, живые (не возвращённые) записи со
    ...
    "auto_reject_sheet_tab": {
        "type": "text", "group": "apps", "label": "📄 Вкладка автоотказов в таблице",
        "prompt": (
            "Как назвать вкладку таблицы, куда бот сам выгружает всех, кого отсеяли правила "
            "автоотказа. Например: 🤖 Автоотказы.\n\n"
            "Пустое значение — вкладка не ведётся.\n\n"
            "Если переименуете вкладку в самой таблице — поменяйте и здесь, иначе бот создаст "
            "новую."
        ),
        "default": "",
    },
    ...
    # Квик 260911-w2m: текст делегату, когда положение "reg_edit_policy" запрещает правку
    # ЕМУ ПРЯМО СЕЙЧАС — три места показа (профиль приложения вместо кнопки «✏️ Изменить
    # анкету», экран анкеты вместо мастера/обзора, ответ бота на вход в правку из чата),
    # появляется только при закрытой правке. Сам тумблер положения — group "toggles", живёт
    # в settings_toggle_rows/admin_sections.SECTIONS, не в _APPS_FIELD_ORDER (см. handlers/
    # admin_settings.py).
    "reg_edit_closed_text": {
        "type": "text", "group": "apps", "label": "✏️ Правка закрыта: текст делегату",
        "prompt": (
            "Что увидит делегат вместо кнопки «Изменить анкету» (профиль, экран анкеты, вход "
            "в правку из чата), пока переключатель «✏️ Правка анкеты делегатом» не в "
            "положении «всегда можно»."
        ),
        "default": (
            "Изменить анкету сейчас нельзя. Если нужно что-то поправить — напишите "
            "менеджеру мероприятия."
        ),
    },
    # Квик 260922-wrg: текст делегату, когда положение "reg_resubmit_after_reject" запрещает
    # повторную подачу ПРЯМО СЕЙЧАС (отклонён в этом сезоне) — показывается в боте (/start
    # возвращенца, rereg_start, finalize_registration) и в Mini App (хаб, экран статуса, форма
    # 409 edit_closed). Сам тумблер положения — group "toggles", живёт в
    # settings_toggle_rows/admin_sections.SECTIONS, не в _APPS_FIELD_ORDER.
    "reg_resubmit_closed_text": {
        "type": "text", "group": "apps", "label": "🔁 Повторная подача закрыта: текст делегату",
        "prompt": (
            "Что увидит отклонённый делегат вместо кнопки «Обновить анкету» (бот и Mini App), "
            "пока переключатель «🔁 Повторная подача после отказа» не в положении «можно». "
            "Отклонённых прошлого сезона (возвращенцев) это не касается — им можно всегда."
        ),
        "default": (
            "К сожалению, повторно подать заявку в этом сезоне нельзя. Если считаете, что "
            "произошла ошибка, — напишите организаторам."
        ),
        # Правка 260922-wrg (владелец): по городам — читается через i18n.tr_setting_for_city /
        # cities.get_setting_for_city, тот же приём, что у прочих делегатских текстов группы
        # "apps" с "per_city": True (см. approve_text/reject_text выше).
        "per_city": True,
    },
    ...
    # копирайтинг-копия.
    "chat_bind_reconcile_start_text": {
        "type": "text", "group": "system", "label": "💬 Сверка после привязки: старт",
        "prompt": (
            "Что админ увидит личным сообщением сразу после привязки чата, пока идёт "
            "сверка состава с Telegram (несколько минут)."
        ),
        "default": "Сверяю состав чата, это несколько минут.",
    },
    "chat_bind_reconcile_done_text": {
        "type": "text", "group": "system", "label": "💬 Сверка после привязки: итог",
        "prompt": (
            "Итог разовой сверки чата после привязки — личным сообщением админу. "
            "Плейсхолдеры: <code>{present}</code> — в чате, <code>{absent}</code> — нет, "
            "<code>{not_found}</code> — не найдены (аккаунт удалён/недоступен боту)."
        ),
        "default": "Готово: {present} в чате, {absent} нет, {not_found} не найдены.",
    },
    ...
    #
    # Соседний тумблер «🔔 Уведомление о заявке» (pending_notify_mode) — про ДРУГОЕ: сообщать
    ...
    # меняя дефолт для остальных событий бота (RusCo и другие форумы не обязаны знать про этот
    # ключ вовсе).
    "reg_submit_digest_max_minutes": {
        "type": "int", "group": "apps", "label": "⏱ Пачка заявок: не позже чем через (мин)",
        "prompt": (
            "Сводка о новых заявках всё равно уйдёт не позже стольки-то минут от ПЕРВОЙ "
            "заявки в очереди, даже если заявки продолжают идти одна за другой и окно тишины "
            "не наступает.\n\n"
            "В МИНУТАХ. Примеры:\n30 = сводка уйдёт не позже получаса от первой заявки в "
            "очереди\n0 — не ограничивать (сводка ждёт тишины сколько угодно, прежнее "
            "поведение)"
        ),
        "default": 0,
    },
    ...
    # (services/daily_digest.py): сколько заявок пришло, кто сколько одобрил и отклонил,
    # сколько сдач проверено и монет начислено. Пара «тумблер + время» разложена так же, как
    # chat_tracking_enabled/chat_refresh_minutes: тумблер строкой раздела «🔧 Управление»,
    # время — соседним ключом группы «🔧 Система», где живут остальные тайминги фоновых джоб.
    # Дефолт OFF: молча начать писать менеджерам каждый вечер нельзя.
    "daily_digest_enabled": {
        "type": "enum", "group": "toggles", "label": "📊 Итоги дня менеджерам",
        "options": ["on", "off"], "prompt": None, "default": "off",
    },
    "daily_digest_time": {
        "type": "text", "group": "system", "label": "📊 Итоги дня: во сколько",
        "prompt": (
            "Во сколько слать менеджерам вечернюю сводку за день. Формат <code>ЧЧ:ММ</code>, "
            "например <code>21:00</code>. Время московское, как везде в боте.\n\n"
            "День считается с 00:00 до момента отправки. Новое время начнёт действовать "
...
def _parse_setting(key, raw):
    ...
    if entry_type == "date_only":
        # Phase 31 (31-03, D-30): дата БЕЗ времени — 'ДД.ММ.ГГГГ'. Отдельный от "date" тип,
        # а не переиспользование его с обрезкой времени: у `payment_deadline` (единственный
        # сегодняшний потребитель "date") время суток осмысленно — по нему планировщик ставит
        # напоминания за 3 дня и за 1 день (T-3/T-1); у даты начала форума времени нет и не
        # будет — подсказка менеджеру обязана совпадать с тем, что реально принимается
        # (CLAUDE.md: пример формата в подсказке — часть контракта с человеком). Если бы этот
        # тип разбирал "%d.%m.%Y %H:%M", менеджер, введя ровно то, что просит подсказка
        # ("15.10.2026"), получил бы молчаливый сброс в default — найденный исследованием
        # баг (T-31-03-01), а не гипотеза.
        try:
