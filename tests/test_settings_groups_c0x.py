...
def test_registry_coverage_event():
    ...
    allowed_types = {
        "toggle", "int", "list", "multi", "date", "date_only", "text", "enum", "photo", "file",
    }
...
def test_render_snapshot_reg(tmp_path):
    ...
    expected_labels = [
        ...
        "🏙 Выбор города: вопрос",
        "🔒 Город: регистрация закрывается", "🔒 Город закрыт: текст", "🔒 Все города закрыты: текст",
        "🔗 Проверенные сайты для ссылки на резюме",
        "📎 Развилка резюме: кнопка «Файл»", "🔗 Развилка резюме: кнопка «Ссылка»",
        "✍️ Развилка резюме: кнопка «Текстом»",
        "🙅 Развилка резюме: кнопка «Нет резюме»",
        "🔗 Ссылка на резюме: маркер «сайт проверенный»",
    ...
    # Владелец 17.09: три новых ключа ветки «Написать текстом» — тоже text с непустым дефолтом.
    # Квик 260923-p37: «🔒 Город закрыт: текст»/«🔒 Все города закрыты: текст» — text с непустым
    # дефолтом (тот же флаг); «🔒 Город: регистрация закрывается» — date_only с default=None,
    # как forum_date, -> «— не задано».
    defaulted_labels = {
        "🏙 Выбор города: вопрос", "🏆 Кейс-чемпионат: пояснение",
        "🔒 Город закрыт: текст", "🔒 Все города закрыты: текст",
        "📝 Мультивыбор: подсказка лимита", "📝 Мультивыбор: счётчик выбранного",
        "📝 Мультивыбор: ошибка превышения лимита",
        "📎 Развилка резюме: кнопка «Файл»", "🔗 Развилка резюме: кнопка «Ссылка»",
        "✍️ Развилка резюме: кнопка «Текстом»",
...
def test_render_snapshot_apps(tmp_path):
    ...
    defaulted_labels = {
        ...
        # Phase 28 (28-09, SU-09): дефолт "несколько" — тоже настоящий текстовый дефолт.
        "⏰ Догонялка: слово вместо числа при сбое",
