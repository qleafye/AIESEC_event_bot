...
# ── Решение Фазы 07.2 ОСОЗНАННО пересмотрено квиком 260919 (P3) ──────────────────────────────

def test_manager_pending_reminder_is_personal_per_recipient_city_scope():
    """РЕШЕНИЕ ФАЗЫ 07.2 («напоминание остаётся глобальным») отменено квиком 260919-u7e (P3,
    находка #03-moderation) — зафиксированное решение, а не забытая правка.

    Что случилось на проде: привязка админ→город (`staff.city`) с тех пор УЖЕ появилась
    (Phase 8/09.1), а рассылка по `config.ADMIN_IDS` — только 7 reg_manager из `staff` вообще
