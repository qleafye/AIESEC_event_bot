...
KNOWN_OVERAGES: dict[str, tuple[int, str]] = {
    ...
    "admin_settings.py": (
        2234,
        "+13 строк (ревью фазы 20: изоляция данных потоков EditSetting) -- вход в "
        "поток задаёт данные целиком (set_data вместо домешивания), а _return_hint_from_state "
        "выбирает поток по СОСТОЯНИЮ через таблицу flows, а не по первому найденному ключу. "
