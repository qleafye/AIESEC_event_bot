...
KNOWN_OVERAGES: dict[str, tuple[int, str]] = {
    "registration.py": (
        ...
        "+55 строк (квик 260919-u7e, находка #3): `_resumable_draft_for` — предикат «жив ли "
        "черновик анкеты» для catch-all вне /start (`handlers/reg_silence_fallback.py`, "
        "молчание бота после рестарта контейнера); переиспользует уже существующие "
        "`_draft_is_fresh`/`reg_resume_ttl_hours`, но не вызывает и не вызывается из `cmd_start` "
        "(та функция слишком плотно сплетена с deep-link/языковым/pre-selection гейтами вокруг "
        "— трогать её ради этого рискованно); потолок поднят до фактического размера. "
        "+2 строки (квик 260919-u7e, находка #2, `recall_keep`): в этой же волне до находки #3 "
        "чинился регресс «Изменить» (33 подтверждённых потери резюме с 15.09) — резюме "
        "восстанавливается из `_prior_answers` набором колонок шага (`columns_for_step(\"resume\")`), "
        "а не оставляется no-op'ом под ложной надеждой на COALESCE `add_user`, который не спасает "
    ...
    "admin_reg_percity.py": (
        ...
        "Приёмка 19.09 (review-260919, «Модерация» находки №2/№3): +3 строки -- "
        "reg_resume_mode_toggle зовёт _refresh_sheet_header (главный триггер шапки листа "
        "пропускал именно этот тумблер, хотя докстринг функции его уже называл), 1130 строк; "
    ...
    "user_actions.py": (
        1765,
        "Потолок поднят 1700->1765 (23.09, сезонный гейт): делегату прошлого сезона игра/монеты/рейтинг/реф-ссылка закрыты до новой анкеты — ensure_current_season + точки вызова в шести хендлерах; вынос в отдельный шов вместе с разрезом игрового блока. "
        ...
        "Потолок поднят 1440->1450 (квик 260919-u7e, находка #3): `reg_handoff_idle_fallback` "
        "получил вторую ветку — черновик держит бот (не приложение), а FSM пуста после "
        "рестарта контейнера — зовёт `handlers.reg_silence_fallback.offer_if_resumable`; вся "
        "логика решения/текста живёт в том шве, здесь только развилка + вызов. "
        "Потолок поднят 1450->1500 (32-06, задача 1): единое правило видимости заданий "
        "амбассадора в `_game_task_list_screen` (visible_tasks_for/sort_tasks_for_ambassador, "
        "чтение user/волн один раз, вставка заголовка блока), слово «без срока» вместо "
        "сырой метки в `_render_game_task_line`, и уборка второй копии `strptime(deadline_at)` "
        "из `mytask_submit_start` — правило живёт в общем `game_labels.py`, здесь только "
        "точки вызова; потолок поднят до фактического размера. "
        "Потолок поднят 1500->1590 (32-06, задача 2): экран рейтинга волны "
        "(_wave_rating_screen/show_wave_rating, callback ambwave) дописан в самый хвост файла "
        "(golden-снапшот роутера требует чистого аппенда) + кнопка «🏅 Рейтинг волны» в списке "
        "заданий; потолок поднят до фактического размера. "
        "Потолок поднят 1590->1700 (32-06, задача 3): путь/выход/возврат амбассадора и экран "
        "«Моя ссылка» как единая точка самообслуживания (_referral_screen + пять новых "
    ...
    "admin_caps.py": (
        ...
        "в очереди бота (appr_flag:*, рядом с остальными appr_*); 1015 -> 1021. "
    ...
    "admin_moderation.py": (
        970,
        "План 32-05 (D-20/D-22): +10 строк — appr_all_yes переведён на claim_approve_all_"
        "with_credits (общий шов с веб-обёрткой claim_approve_all, вместо прямого вызова "
        "database.db.approve_all_pending), плюс строка сводки начислений в тексте "
        "подтверждения менеджеру, когда начисления реально были; 958 -> 968, потолок с "
        "небольшим запасом. "
        "План 31-11 (D-20): +28 строк — чип «только помеченные правилами» в очереди бота: "
        "состояние живёт в FSM (`appr_flagged_only`, тот же принцип, что `appr_skipped`), "
        "прокинуто в get_pending_count/get_pending_users, чип в _appr_card_kb виден только "
        "при включённом модуле правил, пустая отфильтрованная очередь объясняет фильтр "
        "словами, новый хендлер appr_flag_toggle; 930 -> 958, потолок с небольшим запасом. "
        ...
        "Приёмка 19.09 (review-260919, «Модерация» находки №2/№3): +8 строк — блок резюме в "
        "_render_application_card и appr_resume читают moderation_card.resume_summary (общий "
        "источник с веб-карточкой): ветки «ссылка»/«мини-профиль»/маркер потери данных вместо "
        "молчаливого «нет резюме», 899 -> 907; потолок поднят до фактического размера + ~3 "
        "строки запаса. "
        "Правила автоотказа: +23 строки — бейджи пометки/автоотказа и «сменил ответ после "
        "автоотказа» (rule_lines/cleared_line в _render_application_card и их резолв в "
        "_show_current_card), 907 -> 930; потолок поднят до фактического размера + ~5% запаса.",
    ),
}
