...
Hotfix port onto `hotfix-prod` (production code predates the admin_gamification.py split and
the task-archive feature) -- game wizard/point-edit handlers live in `handlers.admin`, not
`handlers.admin_gamification`. Archive-dependent coverage (`db.archive_task`, `archived_at`)
does not apply on this branch and is intentionally NOT ported.

Covers: `task_title()` fallback (decision 2), title/photo DB round-trip (decisions 1/4), the
wizard's new title-first-step / photo-step (decisions 1/4), the point-edit screen («✏️
Название» / «📷 Заменить фото» / «🗑 Убрать фото», decisions 1/4), the delegate task CARD
open/back navigation with and without a photo (decision 5), and that the manager list/
review-card/sheet builders show the title, not a raw text preview (decisions 3/6/8).
ADMIN_CAPS coverage for the new callbacks (decision 8's "moderate_game").

Handlers called DIRECTLY with Fake message/callback doubles -- same convention as every other
phase-9 gamification test file (pytest-asyncio unavailable in this env).
"""
import asyncio
...
def test_game_task_title_step_rejects_empty_and_advances(tmp_path):
    ...
    asyncio.run(admin_mod.game_task_title_step(empty_message, state))
    assert asyncio.run(state.get_state()) == GameTaskCreate.title
    assert "пуст" in empty_message.answers_sent[-1].lower()

    ok_message = FakeMessage(text="  Моё\nзадание  ")
    asyncio.run(admin_mod.game_task_title_step(ok_message, state))
...
def test_game_task_title_step_truncates_over_60_chars(tmp_path):
    ...
    asyncio.run(admin_mod.game_task_title_step(message, state))
...
def test_game_task_photo_step_sets_file_id_and_advances_to_category(tmp_path):
    ...
    asyncio.run(admin_mod.game_task_photo_step(message, state))
...
def test_game_task_photo_skip_clears_file_id_and_advances(tmp_path):
    ...
    asyncio.run(admin_mod.game_task_photo_skip(callback, state))
    assert asyncio.run(state.get_state()) == GameTaskCreate.category
...
def test_game_task_photo_step_invalid_content_reprompts(tmp_path):
    ...
    asyncio.run(admin_mod.game_task_photo_step_invalid(message, state))
...
def test_full_wizard_creates_task_with_title_and_photo(tmp_path):
    _db_ready(tmp_path)
    state = _new_state()
    asyncio.run(admin_mod.game_task_new(FakeCallback("gtnew"), state))
    asyncio.run(admin_mod.game_task_title_step(FakeMessage(text="Итоговое задание"), state))
    asyncio.run(admin_mod.game_task_text_step(FakeMessage(text="Описание задания"), state))
    asyncio.run(admin_mod.game_task_photo_step(
        FakeMessage(photo=[FakePhotoSize("cover1")]), state,
    ))
    asyncio.run(admin_mod.game_task_category_step(FakeCallback("gtcat:Light"), state))
    asyncio.run(admin_mod.game_task_coins_step(FakeMessage(text="10"), state))
    asyncio.run(admin_mod.game_task_proof_step(FakeCallback("gtproof:text"), state))

    confirm_msg = FakeMessage()
    asyncio.run(admin_mod.game_task_proof_done(FakeCallback("gtproof_done", message=confirm_msg), state))
    deadline_msg = FakeMessage(text="01.01.2099 00:00")
    asyncio.run(admin_mod.game_task_deadline_step(deadline_msg, state))
    ...
    asyncio.run(admin_mod.game_task_confirm(confirm_cb, state))
...
def test_game_task_edit_screen_shows_add_photo_when_none(tmp_path):
    ...
    asyncio.run(admin_mod.game_task_edit_screen(callback, _new_state()))
...
def test_gtedittitle_flow_updates_title(tmp_path):
    ...
    state = _new_state()
    asyncio.run(admin_mod.game_task_edittitle_start(FakeCallback(f"gtedittitle:{task_id}"), state))
    ...
    asyncio.run(admin_mod.game_task_edittitle_step(message, state))
...
def test_gteditphoto_flow_replaces_photo(tmp_path):
    ...
    state = _new_state()
    asyncio.run(admin_mod.game_task_editphoto_start(FakeCallback(f"gteditphoto:{task_id}"), state))
    assert asyncio.run(state.get_state()) == GameTaskEdit.photo

    message = FakeMessage(photo=[FakePhotoSize("new_cover")])
    asyncio.run(admin_mod.game_task_editphoto_step(message, state))
    assert asyncio.run(state.get_state()) is None
    task = asyncio.run(db.get_task(task_id))
...
def test_gtremovephoto_clears_photo_no_confirm_needed(tmp_path):
    ...
    asyncio.run(admin_mod.game_task_removephoto(callback))
    task = asyncio.run(db.get_task(task_id))
    assert task["photo_file_id"] is None
...
# ── manager list/review-card/sheet use title (CONTEXT.md decisions 3/6/8) ──────────────────
...
def test_matrix_and_history_column_use_title(tmp_path):
    ...
    headers, _rows = admin_mod._build_game_matrix(tasks, submissions)
    assert "Заголовок" in headers
    assert not any("Полный текст задания подробно" in h for h in headers)

    _h_headers, h_rows = admin_mod._build_game_history(submissions)
...
def test_submission_card_shows_task_line_with_title(tmp_path):
    ...
    card = admin_mod._render_submission_card(rows[0], 1, 1)
