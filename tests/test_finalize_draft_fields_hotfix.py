...
пользователей, деп-линковый `source` подменялся дефолтом «Самостоятельно» (квик 260912:
та же подстановка сегодня подписана «Вопрос не задавался» — здесь оставлена историческая
подпись хотфикса 06.09, не переписываем задним числом), `referrer_id`
