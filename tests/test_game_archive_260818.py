"""Hotfix port (owner: «нужно удалять задания геймы») — task archive/delete (GAME-08).

Ported from `main` commits `9ccb2a0` (data layer: `game_tasks.archived_at`, archive/unarchive/
delete/count accessors, `task_archived_at` exposed on the submission queues) and the `b589344`
merge (manager screens: «📋 Задания» actions «🗄 В архив»/«🗑 Удалить», the «🗄 Архив» screen +
«↩️ Вернуть», two-step confirm gates, moderation-card/sheet-tab archive markers).

Adapted for `hotfix-prod`: gamification lives in `handlers.admin` (no `admin_gamification.py`
split), and the title/photo port (quick 260819-gtl) already reworked the task list into
`_game_tasks_screen()`/`task_title()` — archive actions are added AS EXTRA ROWS on that same
screen, not a full screen rewrite. The resubmit-limit feature (GAME-10, `game_resubmit_limit`,
the delegate «попытка K из N» line) is OUT OF SCOPE for this hotfix and intentionally NOT
ported here — `count_rejected_submissions` exists in `database/db.py` as data-layer plumbing
only (came for free alongside `archive_task`/`delete_task`), unused elsewhere on this branch.

pytest-asyncio is unavailable in this env — every async helper is driven via asyncio.run() and
config.DB_PATH points at a tmp_path file, same convention as
tests/test_game_task_title_photo_260819.py / tests/test_gamification_data_phase9.py.
"""
import asyncio
...
# ── data layer: archive/delete + submission counters ──────────────────────────────────────
...
# ── delegate-side gate: archived task blocks submission ───────────────────────────────────
...
# ── manager «📋 Задания» actions + «🗄 Архив» + return-from-archive + caps ────────────────
...
# ── two-step confirm gates for archive/delete ──────────────────────────────────────────────
...
# ── archive markers (card/matrix/history) ──────────────────────────────────────────────────
