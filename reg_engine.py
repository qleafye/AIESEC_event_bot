...
# Единая точка сборки реф-ссылки (решение владельца, 17.09): и старая кнопка меню в чате,
# и «Хочу свою ссылку» (чат/приложение), и любая будущая поверхность выдачи зовут ЭТУ функцию,
# а не собирают строку сами — один формат `amb_<id>` везде, второй копии литерала не заводим.
# Разбор (`extract_ambassador_ref`/`_extract_referrer_id` в handlers/registration.py) по-прежнему
# принимает и старый числовой `?start=<id>` (уже разосланные ссылки не ломаем), но новые ссылки
# выдаются только в этом формате.
def build_referral_link(bot_username: str, telegram_id: int) -> str:
    return f"https://t.me/{bot_username}?start=amb_{telegram_id}"
...
async def resolve_referrer(referrer_id: int | None) -> int | None:
    ...
    Решение владельца (17.09): применяется к ОБОИМ форматам deep-link — новому `amb_<id>` и
    старому числовому `?start=<id>` (уже разосланные ссылки на проде). Одна точка разбора для
    обоих — `handlers/registration.py::cmd_start` прогоняет через неё что бы ни распарсил,
    отдельной ветки для числового формата больше нет."""
    if not referrer_id:
        return None
