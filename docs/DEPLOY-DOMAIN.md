...
В том же туннеле → вкладка **Public Hostname → Add a public hostname**. Домен `cloud` — одна
запись, а вот `yl26` делится между двумя сервисами (дашборд и Mini App, Phase 19) по пути
запроса — для одного хоста заводятся ДВЕ записи, и **порядок между ними важен**:

| # | Subdomain | Domain    | Type   | Path            | URL                    |
|---|-----------|-----------|--------|-----------------|------------------------|
| 1 | `yl26`    | `<домен>` | `HTTP` | `^/app(/\|$)`   | `miniapp:8001`         |
| 2 | `yl26`    | `<домен>` | `HTTP` | *(пусто)*       | `yl26-dashboard:8000`  |
| — | `cloud`   | `<домен>` | `HTTP` | *(пусто)*       | `nextcloud-app:80`     |

Правило с `Path` (Mini App) обязано стоять **выше** правила без `Path` (дашборд, catch-all) —
Cloudflare матчит правила сверху вниз и останавливается на первом совпадении; если правило `/app`
окажется ниже catch-all, оно никогда не сработает и весь `yl26.<домен>` уйдёт на дашборд. В UI
правила одного хоста перетаскиваются мышью — после добавления второй записи проверить порядок.

URL — имя docker-сервиса и порт, без `http://` и без слэша. У записей без `Path` дополнительные
...
(правило `Path` не создаёт отдельной DNS-записи — это маршрутизация внутри одного хоста).
...
## Шаг 11а. Фактическая схема с relay (с 31.08.2026)

Из РФ соединение cloudflared с CF-edge живёт около минуты, поэтому коннектор туннеля работает
**не на сервере бота, а на зарубежной машине** (is-hosting, `systemctl status cloudflared`), а
origin'ы приходят туда обратным ssh-туннелем с сервера бота (`~/tunnel-relay`, autossh):

| Что | На сервере бота | На машине с cloudflared |
|---|---|---|
| Дашборд | `yl26-dashboard` → `ports: 127.0.0.1:8000:8000` (override) | `-R 127.0.0.1:8000:127.0.0.1:8000` → `/etc/hosts`: `127.0.0.1 yl26-dashboard` |
| Mini App | `miniapp` → `ports: 127.0.0.1:8001:8001` (override) | `-R 127.0.0.1:8001:127.0.0.1:8001` → `/etc/hosts`: `127.0.0.1 miniapp` |

Сеть `edge` при такой схеме не используется; правила в панели CF остаются в виде
`http://yl26-dashboard:8000` и `http://miniapp:8001` — имена резолвятся через `/etc/hosts`.
Проверка до создания правила Path — прямо на машине с cloudflared:
`curl -sI http://miniapp:8001/app/health` → 200 с `content-security-policy`, без `x-frame-options`.
Если это работает, а снаружи `/app` отдаёт 404 с телом дашборда — не хватает только правила Path.

Ловушка: `GET /app/` (со слэшем) отдаёт 404, рабочий адрес — `/app` без слэша; именно его
строит бот. Кнопки входа в приложение появляются/исчезают по тумблеру «Mini App включён» без
перезапуска: `curl /app/api/me` меняет ответ с `503 miniapp_off` на `401 no_auth`.
