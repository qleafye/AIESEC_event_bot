...
📋 Заявки · ❓ Вопросы делегатов · ❓ Частые вопросы · 🧾 Поля карточки заявки · тумблеры
модерации, уведомлений, предотбора, сводки, догонялки и повторной модерации изменённых анкет ·
тексты после подачи («📋 Заявки»).
...
Под каждым ОТВЕЧЕННЫМ вопросом есть кнопка «❓ В FAQ #N» — одним нажатием кладёт пару
«вопрос → ответ» в раздел «❓ Частые вопросы» ниже. Перед сохранением можно поправить и
вопрос, и ответ; повторное нажатие на тот же вопрос не создаёт дубль — бот скажет, что такой
пункт уже есть в FAQ, и назовёт его номер.

#### ❓ Частые вопросы

Список готовых ответов на типовые вопросы делегатов («где, во сколько, сколько стоит, что
взять») — делегат читает его сам, до того как написать «Задать вопрос», и не отвлекает
менеджера, если ответ уже есть. Пункты FAQ показываются и в чате бота (кнопка «❓ Частые
вопросы» в меню делегата), и в Mini App (плитка «❓ Частые вопросы» в хабе делегата).

Экран заводится и правится целиком кнопками:

- **➕ Добавить** — короткий мастер «вопрос → ответ»; «Отмена» на любом шаге ничего не
  создаёт;
- открыв пункт из списка, можно поправить текст вопроса и ответа, переставить пункт
  стрелками ⬆️⬇️, скрыть от делегатов кнопкой «🚫 Скрыть» (пункт остаётся в списке менеджера,
  но делегатам не показывается) и обратно показать «✅ Показать»;
- **🗑 Удалить** — спрашивает подтверждение с текстом вопроса и явно называет, у кого пункт
  пропадёт («у всех городов» или «у делегатов города «Название»»); отменить нельзя.

Тот же список ведётся и в приложении — плитка «❓ Частые вопросы» в разделе «📋 Заявки»
(подробнее — «❓ Частые вопросы в приложении» ниже); список общий, база одна, править можно
из чата бота или из приложения — без разницы, где закончить начатое.

Если включён модуль городов (раздел «22. Города мероприятия») и в шапке экрана выбран
конкретный город — у каждого пункта появляется переключатель «🏙 Только {город} / 🌍 Для
всех городов». Городской пункт с ТЕМ ЖЕ вопросом, что и общий, полностью заменяет его для
делегатов этого города («у Казани своя площадка» — общий ответ про площадку остальных
городов не трогает). При шапке «Все города» или выключенном модуле переключателя нет —
вместо кнопки строка-объяснение.

Пока в FAQ нет ни одного включённого пункта, кнопка «❓ Частые вопросы» у делегата не
рисуется вовсе — появляется сама, как только менеджер добавит и включит первый пункт.

Новые тексты реестра (правятся на экране «🎪 Событие/Медиа», раздел «🎪 Событие» — рядом с
текстами «Задать вопрос»): «Вступление FAQ», «Пустой FAQ», подпись кнопки «Не нашёл ответ» —
все три видит делегат в чате бота перед списком вопросов.
...
| 🎓 Форум СкиллАп | Телефон, ВК, город, образование/ВУЗ/курс/направление, стек, опыт, готовность, цель участия, источник, резюме (развилка файл/ссылка/мини-профиль), кейс-чемпионат, автоматический балл. Плюс настройки события: развилка резюме, пропуск «Откуда узнал» у пришедших по реф-ссылке, предложение своей ссылки после анкеты, догонялка через сутки, лимиты мультивыбора «Стек»/«Цель участия», пороги балла. Оплата — выкл. Разбор по шагам — в шпаргалке, раздел «Первая настройка СкиллАп» |

⚠️ Пресет **перезатирает** текущие тумблеры вопросов. Ставим его первым, донастраиваем после.
Множества правил балла (какие направления считать IT, какие статусы образования —
старшими и т. п.) пресет **не трогает** — они зависят от списков вариантов твоего события,
их отмечаешь галочками сам на экране «🧮 Правила балла» (§7).
...
**Кто считается учащимся** — экран «📋 Вопросы регистрации» → «🎓 Кто считается учащимся»:
список статусов образования (галочками), при которых бот спрашивает ВУЗ/курс/направление.
Пусто — прежнее правило («учусь» = любой статус, начинающийся с «Да»). Сами статусы
образования — редактируемый список в «📝 Регистрация», можно завести любое число вариантов
(СПО, бакалавриат, магистратура, аспирантура, «выпустился недавно», «выпустился давно», «не
учусь» — под форум СкиллАп и любое другое событие).

**Новые вопросы под форум СкиллАп** (все выключены по умолчанию, включаются пресетом
«🎓 Форум СкиллАп» или вручную в «📋 Вопросы регистрации»):

- **«Стек и инструменты»** и **«Опыт работы»**, **«Готовность к работе»** — обычные вопросы
  с вариантами, списки правятся как любые другие («📝 Регистрация» → соответствующий список).
  У мультивыбора («Стек», «Цель участия») можно задать **максимум вариантов** — сколько
  галочек разрешено поставить, число или «без ограничения» (там же, в «📝 Регистрация»).
- **Резюме — три способа вместо одного** (экран «📋 Вопросы регистрации» → кнопка-цикл рядом
  со строкой «Резюме», жми, пока не станет «→ спросить способ» — третье из трёх значений
  после «файл или текст» и «только текст»): делегат сам выбирает, как рассказать о себе —
  прикрепить файл (как раньше), прислать ссылку (hh.ru, GitHub, LinkedIn и другие сайты из
  настраиваемого списка — чужой домен тоже принимается, просто помечается по-другому), либо,
  если резюме нет, бот задаёт три коротких вопроса вместо него (проекты, портфолио — можно
  пропустить, направление развития). Вайтлист доменов правится в «📝 Регистрация» → «🔗
  Проверенные сайты для ссылки на резюме».
- **«Кейс-чемпионат»** — вопрос «Да/Нет» сразу после резюме, с пояснением под ним (оба
  текста правятся как обычные тексты вопроса).
...
(группа **📝 Анкета → «📝 Регистрация»**, подписи «анкета открыта в приложении»/«Забрать
сюда»/«Продолжить в чате») можно поправить как любой другой текст бота — на смысл
переключения это не влияет.

---

### Свой набор вопросов и текстов для города

При включённом модуле городов (§22) набор вопросов, тексты вопросов и режим резюме можно
переопределить отдельно для КАЖДОГО города — так одному городу можно выключить лишний
вопрос или переписать формулировку, не трогая остальные города. Практический разбор кейса
и шаги кнопками — в шпаргалке, раздел «Свой набор вопросов для города».

Переопределяется по городу: включённость каждого вопроса («📋 Вопросы регистрации»),
формулировка каждого вопроса («✏️ Тексты вопросов»), режим резюме (файл-или-текст /
только текст). НЕ переопределяется по городу и остаётся общим для всех городов: варианты
ответов в списках (города, ВУЗы, цели участия и т. п.), оплата, геймификация, напоминания.

**«Нет своего значения» — это «как везде», а не «выключено».** Если у города нет своего
переопределения по какому-то вопросу, экран покажет его состояние из общего набора события
(тумблер включён/выключен, текст — общий). Пометка «своё» рядом с вопросом означает, что
именно у ЭТОГО города переопределение записано; кнопка «↩️ Как везде» стирает переопределение
и возвращает общее значение — это не то же самое, что выключить вопрос.

**Город работает вместе с треком, не вместо него.** Переключатель трека (Полный / Party /
⚡ Краткая) на экранах «📋 Вопросы регистрации» и «✏️ Тексты вопросов» остаётся: правится
набор ВЫБРАННОГО трека в ВЫБРАННОМ городе. Честно о поведении вечеринки и акции: у этих
двух треков городской слой ложится ПОВЕРХ трекового, и если у города нет своего значения
именно для этого трека, вопрос берётся из общего набора СОБЫТИЯ, а не из городского набора
полной формы — переопределение для полной анкеты города не «протекает» само в её вечеринку
или акцию, для них нужно своё переопределение на своём треке.

Кто что правит: менеджер, привязанный к городу (§23), видит и правит только свой город —
общие значения ему недоступны. Пресеты типа события («🎛 Тип события (пресет)») — это
общий bulk-инструмент, он меняет вопросы сразу всем городам и потому из городской шапки не
применяется.
...
обрезки. Тот же набор вопросов правится и в приложении — **⚙️ Настройки → раздел
«📋 Заявки» → «🧾 Поля карточки заявки»**, теми же галочками с подписями вопросов; правка в
любом месте применяется сразу к обеим карточкам.

Вопрос, который анкета спрашивает, а карточка не показывает, помечен на этом экране знаком
**⚠️**, и внизу появляется кнопка **«⚠️ Показать всё, что спрашиваем»** — она включает такие
вопросы разом. Это страховка на случай, когда вопрос анкеты меняют: например, выключили
«Возраст» и включили «Дату рождения» — карточка сама об этом не узнает, у новых заявок не
появится дата рождения, а у старых пропадёт возраст. Кнопка только ДОБАВЛЯЕТ вопросы:
уже включённые она не снимает, даже если анкета их больше не спрашивает — у ранее поданных
заявок ответ на них есть, и прятать его не нужно.

**Автоматический балл заявки.** Включается тумблером **«🧮 Автоматический балл заявки»**
(там же, в «📋 Заявки»). Бот сам считает балл заявки по ответам анкеты — резюме, направление,
статус образования/курс, готовность, опыт, стек — и ставит метку **«IT 3+»**, если направление
и стек тянут на IT-специальность. Балл и метка видны **только менеджеру** — в карточке заявки
(бот и приложение) и отдельными столбцами в Google-таблице; делегат их никогда не видит.

Какие именно варианты ответов приносят балл — экран **«🧮 Правила балла»** (там же, в
«📋 Заявки»): чекбокс-пикеры по текущим спискам вариантов твоего события — «IT-направления»
(из списка «Направление обучения»), «Старшие статусы» (из списка образования), «Готовность
засчитывать», «Опыт засчитывать» — плюс два порога числом: «Курс от» и «Стек от» (сколько
пунктов стека нужно отметить). Пусто — по этому пункту балл не начисляется; пресет «СкиллАп»
эти списки не трогает (§4) — их отмечаешь сам, под свои варианты ответов.

Очередь заявок можно **отсортировать по баллу** — тумблер там же: заявки с более высоким
баллом показываются менеджеру первыми.
...
- **Бросил анкету** → через настраиваемое время (по умолчанию 2 часа, пресет «СкиллАп»
  ставит сутки) приходит напоминание продолжить. Один раз на человека, повторно бот не
  пишет. Текст напоминания и время правятся в «📋 Заявки» → группа «📋 Заявки» («текст и
  порог догонялки»); в текст можно вставить **{remaining}** — бот подставит, сколько
  вопросов осталось конкретному человеку («осталось 3 вопроса»), а если посчитать не
  удалось — человеческое слово вместо числа, делегат никогда не увидит фигурные скобки.
...
**Предложить ссылку сразу после анкеты.** Тумблер **«🎁 Предлагать свою ссылку после
анкеты»** (в «📝 Регистрация», выключен по умолчанию, пресет «СкиллАп» включает) — на
финальном экране (после «Заявка принята») человек видит две кнопки: **«Хочу свою ссылку»**
(получает её отдельным сообщением, без разметки — чтобы ссылку можно было переслать один
в один) и **«Позже»**. Работает и в чате, и в Mini App.

**Не спрашивать «Откуда узнал» у пришедших по ссылке.** Тумблер **«🔗 Не спрашивать
«Откуда узнал» у пришедших по ссылке»** (там же, выключен по умолчанию, пресет «СкиллАп»
включает) — если человек уже пришёл по чьей-то реферальной ссылке, вопрос-источник ему не
задаётся: он и так «Реферальная ссылка», спрашивать второй раз незачем.
...
- Если у города свой набор вопросов (§5), у ЕГО вкладки свои столбцы — не такие, как у
  соседнего города. После смены набора вопросов у города старые строки ЕГО вкладки сами не
  подстроятся под новые столбцы — нужна «♻️ Пересобрать таблицу», она пересобирает каждую
  вкладку под её собственный набор.
...
- Файл автоматически уезжает в облако и **переименовывается по формуле ФИО + Telegram-ник +
  ID + дата-время** (по умолчанию) — у каждого файла своё имя, тёзки и повторные подачи
  друг друга не затирают. Тумблер **«📄 Имя файла резюме: только ID и дата»** (в
  «📝 Регистрация») убирает ФИО и ник из имени файла, если это важно для приватности —
  уникальность имени файла (ID + дата-время) сохраняется в любом случае.
...
Чего действительно нет на 7 сентября 2026:
...
- **Чек-ин на площадке** — право «✅ Чек-ин» в ролях зарезервировано, кнопок за ним нет
  (§22). QR-чекин отдельно не планируется в ближайших фазах.
- **Коды для партнёрского экспорта** — балл и IT-метка (§7) считаются и хранятся человеческими
  подписями (как весь бот), не кодами вроде `spo`/`bachelor` из технического задания партнёра;
  маппинг под конкретный экспорт-формат — отдельная будущая доработка, не в этой версии.
- **Слоты собеседований, отдельные статусы этапов отбора, лидерборд амбассадоров** — следующая
  доработка поверх пресета «СкиллАп», пока не выкачена.
...
Делегат перед анкетой выбирает **город мероприятия** (Москва / СПб / Тюмень). Заявка уезжает
на лист своего города; трек (полная / вечеринка / краткая) работает во всех городах
одинаково. По умолчанию форма во всех городах одна и та же — но набор вопросов, их тексты
и режим резюме можно переопределить отдельно для города (подробно — §5, «Свой набор
вопросов и текстов для города»); если менеджер этого не делал, разницы между городами нет.
...
**Роль** — набор прав, который выдаётся человеку целиком (сейчас три: 🛂 Менеджер
регистраций, 🎮 Менеджер геймификации, 📊 Менеджер статистики). **Право (capability)** —
конкретная возможность внутри админки. У человека может быть несколько ролей одновременно —
права складываются. Сами роли фиксированы в коде, но состав прав каждой роли правится из
бота: кнопка «✏️ Права роли: …» открывает список прав **чекбоксами** — тап переключает,
изменение сохраняется сразу. Технические строки (`moderate_reg` и т.п.) в таблице ниже
приведены только для справки: вводить их вручную больше не нужно и негде.

📊 Менеджер статистики даёт ровно раздел «📊 Данные» (статистика, «📈 Источники», выгрузки) и
вход на веб-дашборд статистики (§26) — ничего больше, никаких настроек и модерации.
...
### Амбассадорский слой: волны и приглашённые

Поверх обычных заданий есть отдельный контур для амбассадоров — тех, кто зовёт людей на
событие по своей реферальной ссылке. Живёт в тех же разделах, чата достаточно — про
приложение см. ограничения в конце раздела.

**Волна** — короткий цикл со своей «историей»: у неё есть только дата начала и дата конца,
номер бот придумывает сам («Волна 1», «Волна 2»…), своего названия у волны нет. Создать:
`/admin` → **🎮 Геймификация** → «🌊 Волны» → «➕ Новая волна» — бот попросит только две даты
(можно одной строкой через «;»: `01.10.2026; 21.10.2026`) и откажется сохранить волну, если
её даты пересекаются с уже существующей, — объяснит словами, а не кодом ошибки. Общая волна
«для всех городов» и волна конкретного города тоже не могут пересекаться по датам: общая
волна физически накрывает все города разом. Дальше — необязательный вводный текст, он уйдёт
амбассадорам в личку на старте волны. Проще не начинать с пустой волны, а нажать
«📋 Скопировать прошлую»: задания переносятся, дедлайны сдвигаются на тот же интервал, что и
сами даты волны, тексты можно поправить уже после. На любом шаге визарда «Отмена» (или
любая другая команда, набранная посреди шага) отменяет шаг целиком — ничего не сохраняется,
бот возвращает на карточку или в список волн.

**Когда задания волны видны участникам.** Только после того, как волну ЗАПУСТИЛИ и
наступила её дата начала. У черновика, у ещё не наступившей волны и у уже закончившейся
задания скрыты от делегатов и амбассадоров, сдачи по ним не принимаются — рейтинг волны
стартует с нуля ровно в момент, когда участники впервые видят её задания, а не раньше.
Поэтому свежескопированную волну-черновик можно спокойно донастраивать сколько угодно —
никто её пока не видит.

**Запуск волны.** Кнопка «▶️ Запустить волну» откажет, если в волне нет ни одного задания
или её дата конца уже прошла, — бот объяснит словами, что поправить. Экран подтверждения
называет точный момент отправки стартового сообщения: если дата начала ещё впереди —
конкретную дату, если уже наступила — «в течение минуты». После подтверждения рассылка
уходит РОВНО на дату начала волны: каждый, кто на тот момент действующий амбассадор,
получает одно сообщение — вводный текст + список заданий с дедлайнами + кнопка «Открыть
задания». Состав получателей бот считает прямо в момент отправки, а не в момент запуска —
значит, кто стал амбассадором уже ПОСЛЕ запуска волны, но ДО её даты начала, тоже получит
это сообщение. Пока рассылка не ушла — даты волны и состав заданий ещё можно свободно менять
на карточке волны. После отправки правится только вводный текст и число призовых мест, а
после того как волна закончилась (дата конца прошла) — только число призовых мест: даты и
задания заперты навсегда, кнопки правки на карточке просто пропадают.

**Задания волны и аудитория.** В визарде задания («➕ Новое задание») два новых кнопочных
шага: к какой волне отнести задание (или «🚫 Вне волн») и кому оно адресовано — «Всем
делегатам» или «Только амбассадорам». На шаге «Волна?» список показывает только те волны,
куда задание сейчас реально можно добавить — своего города (или общую) и с ещё не запертым
составом; волну, куда добавить нельзя, там просто не увидите. Амбассадорские задания лежат
в том же разделе «🎯 Задания» отдельным блоком сверху; обычный делегат их не видит вовсе —
ни в чате, ни в приложении. Дедлайн у задания не обязателен: на шаге срока есть кнопка
«🚫 Без срока» (а у задания волны — ещё и готовый пресет «по умолчанию конец волны»). Волну
удалили — её задания не пропадают: они становятся заданиями «вне волн» со своим прежним
сроком, и напоминание о дедлайне у них по-прежнему сработает.

**Рейтинг волны.** Видят только амбассадоры — кнопка «🏅 Рейтинг волны» под списком заданий.
Тумблер `/admin` → 🎮 Геймификация → «⚙️ Тексты и настройки» → «🏅 Показывать имена в рейтинге
волны» решает, видит ли амбассадор в топе чужие имена или только своё место и отрыв от
призового.

**Итоги волны.** В дату конца волны менеджеру в личку приходит сообщение «Волна N
закончилась, топ такой, на проверке ещё K сдач» с кнопкой «🏁 Объявить итоги». Ту же кнопку
(«🏁 Итоги волны») можно найти и на карточке волны в статусе «ждёт итогов» — если сообщение
потерялось или менеджера назначили позже (а если бот в момент конца волны был выключен —
сообщение придёт сразу после следующего запуска, ничего разбирать вручную не нужно).
Объявление — в два шага с подтверждением («Список призёров после этого не изменится»); если
на границе призовых мест ничья по баллам, экран подтверждения честно предупредит, что
призёров будет больше, чем призовых мест, — призёрами становятся ВСЕ, кто разделил
пограничное место, никого не обделяют произвольным выбором. После подтверждения бот
замораживает список и места навсегда и рассылает итоги всем участникам волны (призёрам —
отдельное поздравление); если рассылка прервётся на середине (сбой, перезапуск бота), она
сама доедет до оставшихся при следующем запуске бота, повторов уже уведомлённым не будет.
Сдача, проверенная уже после объявления, в список призёров не попадает, только в общий
пожизненный зачёт.

**Тексты волны.** В шаблонах стартового сообщения, итогов и поздравления `{wave}` — это
ТОЛЬКО номер волны («2», не «Волна 2»); слово «Волна» пишите в самом тексте, если оно нужно.
Как и везде в боте, лишняя фигурная скобка или незнакомая `{переменная}` в тексте менеджера
больше не ломает рассылку целиком — она просто останется в сообщении как есть, отправка не
прервётся.

**Штраф за просрочку.** Задание можно сдать и после дедлайна — бот всё равно примет сдачу
(дедлайн мягкий, см. выше), но при одобрении применяется фиксированный процент из `/admin` →
🎮 Геймификация → «⚙️ Тексты и настройки» → «⏰ Штраф за сдачу после дедлайна, %». Штраф — один
и тот же в чате и в приложении, что бы менеджер ни выбрал. Делегат видит эту цифру заранее на
карточке задания («после дедлайна — 70 вместо 100»); менеджер — на карточке проверки, ещё до
нажатия «Одобрить», и в приложении тоже, до нажатия. Если менеджер вводит свою сумму вручную
(«своя сумма» — решение по конкретной опоздавшей сдаче), подсказка о штрафе показывается ДО
ввода числа, а после одобрения бот прямо пишет «начислено N из M» — сколько ушло делегату и
сколько было введено.

**Баллы за приглашённого.** `/admin` → 🎮 Геймификация → «⚙️ Тексты и настройки» → «🤝 Баллы за
приглашённого» — сколько баллов действующий амбассадор получает за каждую одобренную заявку
человека, пришедшего по его реферальной ссылке. Начисление разовое: если приглашённому потом
откажут или он отвалится, уже начисленные баллы не пропадают («амбассадор свою работу
сделал»), а повторно за того же человека баллы не капают. По умолчанию сумма — 0, то есть
баллы не начисляются, пока менеджер сам не впишет число больше нуля. В приложении у решения
по заявке есть короткое окно «Отменить» — баллы за приглашённого начисляются только после
того, как это окно закрылось, чтобы промах с «Одобрить» и немедленная отмена не оставляли
баллов за несостоявшееся одобрение.

**«Хочу свою ссылку» у действующего амбассадора.** Кнопка не истекает и может прийти
повторно (например, после того как делегат заново заполнил анкету). Повторное нажатие
ДЕЙСТВУЮЩИМ амбассадором ничего не портит: дата вступления не переписывается, из текущей
волны он не выпадает. Новую дату вступления (и, соответственно, «новичок посреди волны»)
получает только тот, кто перед этим САМ вышел из амбассадоров кнопкой «Перестать быть
амбассадором» и теперь возвращается — это осознанно другой случай, не опечатка.

**Два ограничения этой фазы** — чтобы не искать несуществующую кнопку:

1. Волны и амбассадорские задания настраиваются **только в чате бота**. В приложении своих
   экранов у волн пока нет: веб-редактор заданий спокойно показывает задание «без срока»
   словами и не ломается, но поставить «без срока» или привязать задание к волне можно
   только в чате.
2. Баллы за приглашённого приходят амбассадору только при **боевом** одобрении заявки —
   карточкой модерации, кнопкой «Принять всех» или автоматическим одобрением на финале
   анкеты. Тестовые состояния, которые ставит команда `/uat` на стенде (и генератор
   скриншотов для приёмки), меняют статус заявки напрямую, в обход начисления — если после
   такой команды баллы не пришли, это не поломка, а ожидаемое поведение.
...
- **«✨ Анимации приложения»** — три варианта по кругу: «Авто — по устройству» (движение
  подстраивается под телефон делегата, значение по умолчанию), «Только лёгкие» (только
  проявление, без сдвигов и полос) и «Выключить» (приложение статично, как раньше). Отдельно
  от этой настройки: если у делегата в телефоне включено системное «уменьшение движения»,
  анимации выключены всегда, какой бы вариант ни выбрал менеджер.
- Двенадцать чекбоксов-разделов — какие экраны видны в приложении: 🎯 Задания, 🪙 Монеты,
  🏆 Рейтинг, 👤 Профиль, **📝 Анкета**, **❓ Частые вопросы**, 🎮 Проверка сдач,
  **🗂 Отбор заявок**, **❓ Вопросы делегатов**, 🗂 Задания (менеджер), 📊 Статистика,
  ⚙️ Настройки. Выключенный раздел просто не появляется
  ...
  Выключение «❓ Частые вопросы» прячет в приложении разом два экрана — делегатский список
  FAQ И менеджерский экран его ведения (плитка «❓ Частые вопросы» в разделе «📋 Заявки») —
  сам раздел FAQ в чате бота (§7) продолжает работать как обычно.
...
- **Карточка** — фото делегата (или инициалы, если профиль закрыт), имя, город, бейджи
  ...
  показывать — тот же экран «🧾 Поля карточки заявки» (`/admin` → 📋 Заявки, либо
  ⚙️ Настройки → «📋 Заявки» в приложении), что и у карточки в чате: один набор вопросов на
  все три поверхности, поправил в одном месте — увидел везде.
...
### ❓ Частые вопросы в приложении

Плитка «❓ Частые вопросы» в хабе делегата открывает тот же список, что и кнопка «❓ Частые
вопросы» в меню бота — вопрос раскрывается тапом, ответ показывается под ним; пустой список —
человеческий текст вместо пустого экрана. Городской приоритет ответа тот же, что в чате: если
у делегата включён свой город и для него заведён отдельный пункт с тем же вопросом, он
перекрывает общий.

Менеджер с правом «Модерация заявок» видит в разделе «📋 Заявки» плитку «❓ Частые вопросы» —
полноценный экран ведения того же списка, без перехода в чат: добавить пару «вопрос → ответ»,
поправить текст вопроса и ответа, переставить пункт стрелками, скрыть от делегатов и вернуть
обратно, переключить «только мой город ↔ все города» (переключатель виден, только если
учётка менеджера привязана к конкретному городу — иначе строка-объяснение вместо мёртвой
кнопки) и удалить с подтверждением, которое называет вопрос и у кого именно пункт пропадёт.
Кнопка «В FAQ» под отвеченным вопросом на экране «❓ Вопросы делегатов» (см. выше, «📋 Заявки
в приложении») остаётся быстрым способом завести пункт прямо из журнала — открывает форму с
предзаполненными вопросом и ответом. Список один и тот же на всех трёх поверхностях (чат
бота, экран ведения в приложении, кнопка «В FAQ») — правки видны сразу везде.
...
С этого раздела у анкеты появился второй, новый вид — «Новая анкета». Старая анкета никуда
не делась и остаётся дефолтом: пока менеджер сам не включит переключатель, ни делегат, ни
менеджер не увидят никакой разницы.

### Старая и новая анкета — в чём разница

- **Старая анкета** — сегодняшний вид: вопрос за вопросом, обычные кнопки в чате, обычные
  поля в приложении. Работает так же, как работала всегда.
- **Новая анкета** — тот же порядок вопросов и те же ответы (данные и таблица не меняются),
  но каждый вопрос выглядит современнее: списки — крупными плитками с пояснением, город/ВУЗ —
  поиском с подсказками, «Образование» — одной карточкой вместо четырёх вопросов подряд,
  «Опыт и проекты» — кнопкой «Добавить ещё», а после отправки — полноэкранный статус заявки
  вместо строчки в профиле.
- Переключение работает **на ходу, без перезапуска бота** — можно включить, посмотреть,
  выключить обратно одной кнопкой. Делегат, который в момент переключения заполняет анкету,
  просто продолжает с того же вопроса в новом виде — ответы не пропадают.

### Где включать

`/admin` → раздел **📝 Анкета** → экран **«Анкета мероприятия»** (то же самое видно и в
Mini App, экран раздела «📝 Анкета»). Наверху — большой переключатель «Старая анкета /
Новая анкета», под ним — список из восьми более мелких настроек, каждая для одного элемента
новой анкеты.

### Восемь элементов и что каждый даёт

Каждый элемент можно выключить отдельно — вопрос при этом не ломается, а становится проще
(ровно так, как выглядел в старой анкете этот же элемент). Порядок — как на экране менеджера:

1. **Чипы частых значений** — под вопросом сразу видно восемь самых популярных ответов этого
   сезона (город, ВУЗ и похожие вопросы). Выключено — чипов нет, остаётся обычный список/поле.
2. **Поиск в справочнике** — город и ВУЗ ищутся по первым буквам вместо пролистывания всего
   списка. Выключено — короткий список кнопок, как раньше.
3. **Образование одним экраном** — ВУЗ, курс и программа обучения на одной карточке вместо
   четырёх вопросов подряд. Выключено — снова четыре отдельных вопроса.
4. **Повторяемые блоки** — кнопка «Добавить ещё» у «Опыта и проектов» (можно рассказать про
   несколько мест/активностей). Выключено — только один ответ, как раньше.
5. **Счётчик лимита в заголовке** — у вопросов с ограничением выбора («выбери до трёх») сразу
   видно «2 из 3» в заголовке. Выключено — лимит виден только в подсказке под полем.
6. **Экран статуса заявки во всю ширину** — после отправки анкеты у делегата отдельный
   красивый экран «На проверке / Одобрена / Отклонена» вместо строчки в профиле, и короткая
   плашка со статусом на главном экране (хабе). Выключено — прежняя строчка в профиле.
7. **Настройки в шапке анкеты** — шестерёнка над вопросом: делегат сам выбирает тему
   (светлая/тёмная/как в Телеграме), язык анкеты (если включён модуль языка) и вибрацию —
   это его личный выбор, на ответы и на то, что видит менеджер, не влияет. Выключено —
   шестерёнки нет вовсе.
8. **Вибрация** — короткий отклик телефона на выбор варианта и на ошибку. Выключено — анкета
   работает беззвучно/без вибрации.

Под списком — вторичная кнопка «Посмотреть анкету глазами делегата»: открывает анкету так,
как её увидит делегат, без необходимости просить кого-то показать телефон.

### Справочник ВУЗов и городов — очередь «Другое»

Новая анкета ищет город и ВУЗ по встроенному справочнику (без интернета, без сторонних
сервисов). Если делегат не нашёл свой вариант в списке, он может вписать его сам — такой
ответ **не теряется**, а попадает в очередь менеджера на экране бота **«📚 Справочники»**
(`/admin` → 📝 Анкета → «📚 Справочники»):

- **Очередь «Другое»** — список сырых ответов делегатов, которых нет в справочнике. Каждый
  можно **влить как псевдоним** (найти в справочнике похожий вариант и присоединить к нему)
  или **отклонить** (если это опечатка/мусор, а не реальный ВУЗ/город).
- **Закрепить чипы** — до восьми вариантов справочника можно закрепить вручную, чтобы они
  всегда показывались первыми в чипах (например, вузы своего города) — остальные места в
  чипах занимают самые частые ответы этого сезона.
- У каждого списка справочника (ВУЗ, город) есть три отдельные настройки — «чипы», «поиск»,
  «разрешить свой вариант» — на экране правки самого списка (`/admin` → 📝 Анкета →
  правка списка «ВУЗ»/«Город»). Выключенный «свой вариант» означает, что делегат обязан
  выбрать что-то из списка — свободный текст в этом случае не принимается.

### Если что-то пошло не так

Большой переключатель наверху экрана «Анкета мероприятия» возвращает всё к старому виду
одним нажатием — ответы делегатов при этом никуда не пропадают (данные и таблица общие для
обоих видов анкеты).
