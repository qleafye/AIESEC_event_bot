...
приложение см. первое ограничение ниже.
...
её даты пересекаются с уже существующей, — объяснит словами, а не кодом ошибки. Дальше —
необязательный вводный текст, он уйдёт амбассадорам в личку на старте волны. Проще не
начинать с пустой волны, а нажать «📋 Скопировать прошлую»: задания переносятся, дедлайны
сдвигаются на тот же интервал, что и сами даты волны, тексты можно поправить уже после.

**Что происходит после запуска.** Кнопка «▶️ Запустить волну» на карточке ставит рассылку
РОВНО на дату начала волны: каждый, кто на тот момент действующий амбассадор, получает одно
сообщение — вводный текст + список заданий с дедлайнами + кнопка «Открыть задания». Состав
получателей бот считает прямо в момент отправки, а не в момент запуска — значит, кто стал
амбассадором уже ПОСЛЕ запуска волны, но ДО её даты начала, тоже получит это сообщение. Пока
рассылка не ушла — даты волны и состав заданий ещё можно свободно менять на карточке волны.
После отправки правится только вводный текст и число призовых мест, а после того как волна
закончилась (дата конца прошла) — только число призовых мест: даты и задания заперты
навсегда, кнопки правки на карточке просто пропадают.
...
делегатам» или «Только амбассадорам». Амбассадорские задания лежат в том же разделе
«🎯 Задания» отдельным блоком сверху; обычный делегат их не видит вовсе — ни в чате, ни в
приложении. Дедлайн у задания не обязателен: на шаге срока есть кнопка «🚫 Без срока» (а у
задания волны — ещё и готовый пресет «по умолчанию конец волны»).
...
потерялось или менеджера назначили позже. Объявление — в два шага с подтверждением («Список
призёров после этого не изменится»); после подтверждения бот замораживает список призёров и
рассылает итоги всем участникам волны (призёрам — отдельное поздравление). Сдача, проверенная
уже после объявления, в список призёров не попадает, только в общий пожизненный зачёт.
...
🎮 Геймификация → «⚙️ Тексты и настройки» → «⏰ Штраф за сдачу после дедлайна, %». Делегат видит
эту цифру заранее на карточке задания («после дедлайна — 70 вместо 100»), менеджер — на
карточке проверки, ещё до нажатия «Одобрить».
...
баллы не начисляются, пока менеджер сам не впишет число больше нуля.
