...
по своей ссылке. Подробности и два ограничения фазы (волны только в чате, баллы за
приглашённого — только при боевом одобрении) — [ADMIN_GUIDE.md, §24](ADMIN_GUIDE.md#24-геймификация-задания-и-коины).

1. **Создать волну** — `/admin` → **🎮 Геймификация** → «🌊 Волны» → «➕ Новая волна» → две
   даты одной строкой через «;» (`01.10.2026; 21.10.2026`). Была прошлая волна — жми
   «📋 Скопировать прошлую» вместо пустой: задания и дедлайны переедут сами.
2. **Завести задания волны** — там же → «🎯 Задания» → «➕ Новое задание» → на шаге «Волна?»
   выбрать нужную волну (или «🚫 Вне волн»), на шаге «Кому это задание?» — «Всем делегатам»
   или «Только амбассадорам».
3. **Запустить волну** — «🌊 Волны» → нужная волна → «▶️ Запустить волну». В дату начала
   волны каждый действующий на тот момент амбассадор получит одно сообщение со списком
   заданий.
4. **Объявить итоги** — в дату конца волны бот сам пришлёт менеджеру личное сообщение с
   ...
   статусе «ждёт итогов», если сообщение потерялось. Подтверди — список призёров дальше не
   меняется.
...
| Начислять баллы за приглашённых амбассадором | `/admin` → **🎮 Геймификация** → «⚙️ Тексты и настройки» → «🤝 Баллы за приглашённого» (0 = выключено) |
| Настроить штраф за просрочку задания | Там же → «⏰ Штраф за сдачу после дедлайна, %» (0 = без штрафа) |
| Скрыть имена в рейтинге волны от других амбассадоров | Там же → тумблер «🏅 Показывать имена в рейтинге волны» |
