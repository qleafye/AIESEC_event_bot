...
async def credit_for_approved(invitee_id: int, *, changed_by: int | None = None) -> dict | None:
    ...
    3. Пригласивший существует и `is_ambassador == 1` ПРЯМО СЕЙЧАС (D-37: обычный делегат со
       старой реф-ссылкой и человек, вышедший из амбассадоров, ничего не получают;
       `is_ambassador_candidate` — старое поле анкеты «хочу быть амбассадором», к текущему
       статусу отношения не имеет и здесь НЕ проверяется).
