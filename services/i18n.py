...
def tr(text, lang: str, tr_map: dict[str, str]):
    ...
    Тождественный возврат при `lang != "en"` — ЖЁСТКОЕ требование, не стилистика:
    ...
    текст байт-в-байт. Поэтому при любом `lang`, кроме `"en"` (включая `"ru"`, `"ask"` и
    посторонние значения — и на пустом/`None` тексте, fail-soft на любом языке), функция
    отдаёт ТОТ ЖЕ объект `text`, а не копию и не `str(text)`. `"ask"` здесь — не «язык не
    русский», а «язык ещё не выбран»: пока делегату не показали кнопки выбора, любой текст
    (в т.ч. рукописный ярус A) обязан оставаться русским, иначе `resolve_lang`-ступень 4
    («ask» до первого рендера) тихо протекает в английские подсказки при живом
    `language_code` клиента, отличном от "ru".

    Порядок резолюции при `lang == "en"`:
