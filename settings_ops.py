...
HTML_SETTINGS = {
    ...
    # Quick 260906-8uq: faq_ask_button_text — подпись inline-кнопки (Telegram не парсит
    # разметку в тексте кнопки) — сюда НЕ входит, только сообщения экрана FAQ.
    "faq_intro_text", "faq_empty_text",
...
PREVIEW_SAMPLES: dict[str, str] = {
    ...
    "selected": "2",
    "domain": "github.com",
    # Phase 28 (28-09, SU-09): «осталось {remaining} вопросов» в тексте догонялки — образец
    # для превью в веб-настройках (сам дефолт nudge_text плейсхолдер не содержит, менеджер
    # добавляет его сам).
    "remaining": "3",
    # Phase 30 (30-03/30-04): новые типы шагов — {entity} название справочника в lookup/multi
    # («Впиши {entity} сам…»), {left} остаток лимита мультивыбора в заголовке-подсказке,
    # {noun} название повторяемого блока («Добавить ещё {noun}»), {query} строка поиска
    # lookup без совпадений.
    "entity": "город",
    "left": "2",
    "noun": "проект",
    "query": "спбгу",
    # Phase 30 (30-05): обзор перед отправкой — {list} названия пропущенных необязательных
