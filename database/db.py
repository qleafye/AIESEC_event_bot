...
async def init_db():
    async with aiosqlite.connect(config.DB_PATH) as db:
        ...
        # Hotfix (GAME-08): NULL = active, non-NULL timestamp = archived (delegate never
...
async def list_active_tasks(*, city_scope=None, include_null: bool = True) -> list[dict]:
    ...
    Hotfix (GAME-08): always excludes archived tasks (archived_at IS NOT NULL) — a delegate
    must never see or be able to submit to an archived task, regardless of city_scope. The
    base `archived_at IS NULL` clause is unconditional; the city fragment (if any) is
    appended via AND."""
...
async def list_all_tasks(*, city_scope=None, include_null: bool = True) -> list[dict]:
    """Hotfix (GAME-08): deliberately NOT filtered by archived_at — the manager screen
...
# ── Hotfix (GAME-08): archive/delete + submission counters ───────────────────────────────
...
async def delete_task(task_id: int) -> bool:
    ...
    separate Python read-then-decide step — a manager deleting a task the same second a
    delegate submits to it must never silently drop that submission's parent row."""
    async with aiosqlite.connect(config.DB_PATH) as db:
        cursor = await db.execute(
...
async def count_rejected_submissions(task_id: int, user_id: int) -> int:
    """Rejected-only count for one (task, user) pair. Model is get_active_submission's WHERE
    shape, COUNT instead of a single row. Not wired to a resubmit-limit gate on this branch
    (deferred), kept here as plain data-layer plumbing so a future port has no db.py work
    left to do."""
    async with aiosqlite.connect(config.DB_PATH) as db:
        async with db.execute(
