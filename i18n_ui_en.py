...
UI_EN: dict[str, str] = {
    ...
    # Квик 260917-en (живая проверка 17.09): кнопка «📱 Заполнить в приложении» под приветствием
    # (handlers/registration.py::_reg_form_cta_kb) — та же причина и тот же приём, что у
    # «Открыть приложение» выше: reg_form_cta_text технически в корпусе (группа reg,
    # машинный перевод есть), но подпись короткая и постоянно на виду в UAT — рукописный
    # перевод дефолта надёжнее. Ключ без эмодзи — «📱 » снимается split_leading_symbols'ом.
    ...
    # ── Квик 260917-en (живая проверка 17.09, доп. находка «а»): машинный перевод дал «All
    # right, start registering..» — двойная точка (артефакт движка) и неестественная фраза;
    ...
    # Сезонный гейт игры/монет в чате (23.09): делегату прошлого сезона.
    "Нажми /start, чтобы обновить анкету.": "Tap /start to update your application.",
}
