...
# Каноничный порядок ступеней воронки — тот же, что в `queries.funnel()`. Две ступени
# необязательные — тумблер `payment_enabled` («Оплатили») и, с Phase 31 (31-07, D-27),
# `reject_rules_enabled` («🤖 Автоотказ», на позиции ПОСЛЕ «На модерации» — тот же порядок,
# что в `queries.funnel()`); у события без включённого модуля её просто нет. Обе строки
# сборки сравнения (`funnel_table`/`present_labels` ниже) обязаны это переживать без дыр в
# таблице — тот же механизм, что уже у «Оплатили».
_CANONICAL_FUNNEL_LABELS = (
    "Зашли", "Начали анкету", "Дошли до конца", "На модерации", "🤖 Автоотказ", "Одобрено",
    "Оплатили",
)
...
def _funnel_missing_reason(label: str) -> str:
    ...
    if label == "🤖 Автоотказ":
        return "у этого события правила автоотказа выключены"
