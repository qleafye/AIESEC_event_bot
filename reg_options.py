...
# Подпись, которую `reg_engine.with_defaults` подставляет в `users.source`, когда шага
# «Откуда узнал» в треке делегата вообще не было (party-гости и т.п.) и реф-ссылки нет.
# Это НЕ вариант ответа делегата — в DEFAULT_SOURCE_OPTIONS её нет и кнопкой её выбрать
# нельзя. Не путать с TRANSPORT_OPTIONS ниже: там «Самостоятельно» — настоящий ответ
# делегата в ДРУГОЙ колонке (users.transport, шаг «Трансфер»).
SOURCE_NOT_ASKED = "Вопрос не задавался"

# Прежняя подпись той же подстановки (до квика 260912) — держим только для распознавания
# уже накопленных строк: миграция в database/db.py и инструмент ремонта
# tools/backfill_finalize_fields.py должны узнавать обе подписи одной сущности.
SOURCE_NOT_ASKED_LEGACY = "Самостоятельно"
