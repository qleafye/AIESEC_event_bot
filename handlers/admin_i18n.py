...
async def admin_i18n_seed(callback: types.CallbackQuery):
    ...
    `bulk_seed` вернёт 0 для тех, что уже там, уже переведены (машинно или руками), или под
    ручной правкой — Квик 260917-en расширил пропуск с «только manual» на «любой существующий
    перевод»). Они не пересекаются по построению (`reset_translation_attempts` трогает только
    уже существующие строки, `bulk_seed` считает только новые вставки), поэтому суммарный
    «нечего делать» — это ровно оба нуля, а не эвристика."""
