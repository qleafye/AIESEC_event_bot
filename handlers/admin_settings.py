...
_REG_FIELD_ORDER = [
    ...
    # после экрана выбора города, per-city правка через generic «Для города…».
...
FILE_FIELDS = [
    ("reg_bonus", "🎁 Бонус за регистрацию", (
        "Отправьте файл или фото бонуса (можно с подписью) — бот пришлёт его делегату сразу "
        "после отправки анкеты, пока включён тумблер «🎁 Бонус за регистрацию»."
    )),
]
