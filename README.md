...
(задания, сдача с файлом альбомом, профиль, коины, рейтинг) и менеджера (проверка сдач,
...
переключателя) и пресеты на экране «⚙️ Настройки → 🎨 Оформление → 🎭 Пресеты и ручки
оформления»: акцент, шрифтовая пара, логотип и уровень анимации собираются в CSS на лету
(`web_theme.py`), без правки файлов.
