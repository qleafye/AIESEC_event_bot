...
- Переезд остатков настроек из `.env` в админку — почти закрыт: города заводятся экраном
  «🏙 Города» (`EVENT_CITIES` — только seed при пустой базе), тайминги прокси в реестре.
