...
[YouLead'26](https://youlead26.ru/) идёт через него. Сейчас в работе вторая версия — реестр настроек, города, роли менеджеров,
геймификация.

**Сделано к 19 августа**
...
- Все тексты, которые видит делегат, — в реестре настроек (`SETTINGS_SCHEMA`), правятся
  из «⚙️ Настройки»; хардкода делегатских текстов в коде не осталось.
...
| Тесты | pytest, ~2080 тестов без обращений к внешним сервисам, CI на GitHub Actions |
...
`cities`, дальше города живут в БД и правятся экраном «⚙️ Настройки → 🏙 Города» (добавить,
...
(задания, сдача с файлом, профиль, коины, лидерборд) и менеджера (проверка сдач, статистика
геймы, задания менеджера — список/карточка/правки/создание/архив). Открывается прямо внутри
Telegram текстовой кнопкой «📱 Приложение» или кнопкой меню чата — отдельного логина не требует,
Telegram сам передаёт приложению личность открывшего. Inline-интерфейс бота остаётся основным
путём: Mini App — дополнительный, более удобный для длинных списков и загрузки файлов способ
делать то же самое, а не замена команд бота.

Включается и настраивается менеджером в боте: ⚙️ Настройки → 🎨 Оформление — тумблер «включено»,
видимость разделов, акцентный цвет, логотип. Работает как sidecar `miniapp` того же
`docker-compose.yml` (тот же образ, что и `bot`), снаружи смотрит через тот же Cloudflare Tunnel
по пути `/app*` на домене дашборда. Деплой сервиса и правило маршрутизации — там же, в
...
│   ├── admin_settings.py    # ⚙️ Настройки: экраны из SETTINGS_SCHEMA, вкладки таблицы, выгрузки
...
├── tests/                   # pytest, ~2080 тестов; conftest.py фиксирует порядок импорта хендлеров
...
текстами). Экраны админки «⚙️ Настройки» собираются из `SETTINGS_GROUPS`
(`handlers/admin_settings.py`): 🎪 Событие/Медиа, 📝 Регистрация, 📄 Вкладки таблицы,
💳 Оплата, 🎉 Party, 📋 Согласия, 🎮 Геймификация, 🔧 Система.
...
Менеджер правит их из «⚙️ Настройки» по группам, без разработчика. Примеры ключей:
...
~2080 тестов, полный прогон 10–20 минут в зависимости от машины: чистые хелперы (парсинг
