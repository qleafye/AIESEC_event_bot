...
Бот держит и форумы (YouLead), и конференции (RusCo/Summit): код один, различия между
...
Первая версия отгружена 24 июля 2026, с тех пор бот в проде: регистрация на YouLead'26
идёт через него. Сейчас в работе вторая версия — реестр настроек, города, роли менеджеров,
геймификация.

**Делается прямо сейчас**

- Геймификация (задания, сдачи, коины). Механика и модерация работают, но UI делегата и
  менеджера сырой: экраны заданий, сдачи, баланса и карточки модерации ждут редизайна.
- Переезд остатков настроек из `.env` в админку. Список городов пока задаётся переменной
  `EVENT_CITIES` и требует рестарта — станет экраном в боте.

**Дальше по плану**

- Дашборд статистики регистраций: отдельная веб-страница, read-only, с разрезами по
  городам, трекам, статусам, воронке и ответам анкеты.
- Разбиение `handlers/admin.py` (6.8k строк) и `handlers/registration.py` (3.1k) —
  разрослись до god-файлов, поведение при разбиении меняться не должно.
- Узнавание вернувшегося делегата: префилл ответов прошлого сезона по `telegram_id`.
