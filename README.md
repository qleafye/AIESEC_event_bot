...
экран «⚙️ Настройки» — весь правимый реестр бота с поиском и пакетным сохранением, паритет с
`/admin` сторожат тесты).
...
переключателя) и пресеты на экране `/admin` → 🔧 Управление → «🎨 Оформление» →
«🎭 Пресеты и ручки оформления»: акцент, шрифтовая пара, логотип и уровень анимации собираются в CSS на лету
